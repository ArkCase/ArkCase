/*! angular-dashboard-framework 0.1.0 (2014-07-25) */
angular.module("ui.bootstrap", ["ui.bootstrap.transition", "ui.bootstrap.collapse", "ui.bootstrap.accordion", "ui.bootstrap.alert", "ui.bootstrap.bindHtml", "ui.bootstrap.buttons", "ui.bootstrap.carousel", "ui.bootstrap.dateparser", "ui.bootstrap.position", "ui.bootstrap.datepicker", "ui.bootstrap.dropdown", "ui.bootstrap.modal", "ui.bootstrap.pagination", "ui.bootstrap.tooltip", "ui.bootstrap.popover", "ui.bootstrap.progressbar", "ui.bootstrap.rating", "ui.bootstrap.tabs", "ui.bootstrap.timepicker", "ui.bootstrap.typeahead"]), angular.module("ui.bootstrap.transition", []).factory("$transition", ["$q", "$timeout", "$rootScope", function ($q, $timeout, $rootScope) {
    ...
    return {
        ...
        link: function (scope, element, attrs, ngModel) {
            function combineCallbacks(first, second) {
                return second && "function" == typeof second ?
                function (e, ui) {
...
}]), angular.module("adf").directive("adfWidgetContent", ["$log", "$q", "$sce", "$http", "$templateCache", "$compile", "$controller", "$injector", "dashboard", function ($log, $q, $sce, $http, $templateCache, $compile, $controller, $injector, dashboard) {
...
}]), angular.module("adf").directive("adfWidget", ["$log", "$modal", "dashboard", function ($log, $modal, dashboard) {
...
}]), angular.module("adf").directive("adfDashboard", ["$rootScope", "$log", "$modal", "dashboard", function ($rootScope, $log, $modal, dashboard) {
    function copyWidgets(source, target) {
        if (source.widgets && source.widgets.length > 0) for (var w = source.widgets.shift(); w;) target.widgets.push(w), w = source.widgets.shift()
    }
...
}.call(this), angular.module("sample", ["adf", "sample.widgets.news", "sample.widgets.randommsg", "sample.widgets.weather", "sample.widgets.markdown", "sample.widgets.linklist", "sample.widgets.github", "sample.widgets.myTasks", "LocalStorageModule", "structures", "sample-01", "sample-02", "ngRoute"]).config(["$routeProvider", "localStorageServiceProvider", function ($routeProvider, localStorageServiceProvider) {
...
}]), angular.module("sample-01", ["adf", "LocalStorageModule"]).controller("sample01Ctrl", ["$scope", "localStorageService", function ($scope, localStorageService) {
    var name = "sample-01",
...
}]), angular.module("sample-02", ["adf", "LocalStorageModule"]).controller("sample02Ctrl", ["$scope", "localStorageService", function ($scope, localStorageService) {
    var name = "sample-02",
        model = localStorageService.get(name);
    model || (model = {
        title: "Sample 02",
...
}]), angular.module("structures", ["adf"]).config(["dashboardProvider", function (dashboardProvider) {
...
}]), angular.module("sample.widgets.news", ["adf.provider"]).value("newsServiceUrl", "https://ajax.googleapis.com/ajax/services/feed/load?v=1.0&callback=JSON_CALLBACK&q=").config(["dashboardProvider", function (dashboardProvider) {
    ...
    $scope.feed = feed
}]), angular.module("sample.widgets.myTasks", ["adf.provider"]).value("myTasksServiceUrl", "https://ajax.googleapis.com/ajax/services/feed/load?v=1.0&callback=JSON_CALLBACK&q=").config(["dashboardProvider", function (dashboardProvider) {
    dashboardProvider.widget("myTasks", {
        ...
        resolve: {
            feed: function (myTasksService, config) {
                return config.url ? myTasksService.get(config.url) : void 0
            }
        },
        edit: {
...
}]).service("myTasksService", ["$q", "$http", "myTasksServiceUrl", function ($q, $http, myTasksServiceUrl) {
    return {
        get: function (url) {
            var deferred = $q.defer();
            return $http.jsonp(myTasksServiceUrl + encodeURIComponent(url)).success(function (data) {
                data && data.responseData && data.responseData.feed ? deferred.resolve(data.responseData.feed) : deferred.reject()
            }).error(function () {
                deferred.reject()
            }), deferred.promise
...
}]).controller("myTasksCtrl", ["$scope", "feed", function ($scope, feed) {
    $scope.feed = feed
}]), angular.module("sample.widgets.weather", ["adf.provider"]).value("weatherServiceUrl", "http://api.openweathermap.org/data/2.5/weather?units=metric&callback=JSON_CALLBACK&q=").config(["dashboardProvider", function (dashboardProvider) {
    ...
    $scope.data = data
}]), angular.module("sample.widgets.linklist", ["adf.provider"]).config(["dashboardProvider", function (dashboardProvider) {
...
Showdown.converter = function (converter_options) {
...
}), angular.module("sample.widgets.markdown", ["adf.provider", "btford.markdown"]).config(["dashboardProvider", function (dashboardProvider) {
...
}]), angular.module("sample.widgets.randommsg", ["adf.provider"]).config(["dashboardProvider", function (dashboardProvider) {
    ...
    function p() {
        var b, c, a = arguments,
            d = a.length;
        for (b = 0; d > b; b++) if (c = a[b], c !== t && null !== c) return c
    }
    function A(a, b) {
    ...
    H.Highcharts ? oa(16, !0) : S = H.Highcharts = {}, bb = function (a, b, c) {
        ...
        init: function (a, b, c) {
            ...
            if (a.shift = 0, b.length) for (a = c.length; b.length < a;) d = [].concat(b).splice(b.length - f, f), e && (d[f - 6] = d[f - 2], d[f - 5] = d[f - 1]), b = b.concat(d);
        ...
        H.HighchartsAdapter = H.HighchartsAdapter || a && {
            init: function (b) {
                ...
                e = a.cssHooks.opacity, a.extend(a.easing, {
                    ...
                    set: e
                } : d.d = e, this.each = Array.prototype.forEach ?
                function (a, b) {
    ...
    G.prototype = {
        ...
        css: function (a) {
            var e, f, b = this.styles,
                ...
                d = this.element,
                g = "";
            if (e = !b, a && a.color && (a.fill = a.color), b) for (f in a) a[f] !== b[f] && (c[f] = a[f], e = !0);
            if (e) {
        ...
        add: function (a) {
            var g, h, b = this.renderer,
                ...
                e = this.element,
                f = this.zIndex;
            if (a && (this.parentGroup = a), this.parentInverted = a && a.inverted, void 0 !== this.textStr && b.buildText(this), f && (c.handleZ = !0, f = z(f)), c.handleZ) for (a = d.childNodes, g = 0; g < a.length; g++) if (b = a[g], c = F(b, "zIndex"), b !== e && (z(c) > f || !s(f) && s(c))) {
        ...
        var ha = {
            ...
            color: function (a, b, c, d) {
                ...
                if (a && a.linearGradient ? i = "gradient" : a && a.radialGradient && (i = "pattern"), i) {
                    ...
                    }), "fill" === c) if ("gradient" === i) c = m.x1 || m[0] || 0, a = m.y1 || m[1] || 0, n = m.x2 || m[2] || 0, m = m.y2 || m[3] || 0, D = 'angle="' + (90 - 180 * V.atan((m - a) / (n - c)) / na) + '"', t();
                    else {
                        var w, j = m.r,
                            r = 2 * j,
                            v = 2 * j,
    ...
    ga && (S.CanVGRenderer = Z = function () {
        ...
        defaultLabelFormatter: function () {
            ...
            else if (f && a >= 1e3) for (; f-- && g === t;) c = Math.pow(1e3, f + 1), a >= c && null !== e[f] && (g = Ga(b / c, -1) + e[f]);
        ...
        else if (a >= .08) for (var h, i, j, k, l, f = U(b), e = a > .3 ? [1, 2, 4] : a > .15 ? [1, 2, 4, 6, 8] : [1, 2, 3, 4, 5, 6, 7, 8, 9]; c + 1 > f && !l; f++) for (i = e.length, h = 0; i > h && !l; h++) j = za(ja(f) * e[h]), j > b && (!d || c >= k) && k !== t && g.push(k), k > c && (l = !0), k = j;
    ...
    kb.prototype = {
        ...
        colorizeItem: function (a, b) {
            ...
            if (d && d.css({
                ...
                if (g && f.isMarker) for (j in i.stroke = h, g = a.convertAttribs(g)) d = g[j], d !== t && (i[j] = d);
                f.attr(i)
            }
        },
        ...
        redraw: function (a) {
            var g, h, b = this.axes,
                ...
                n = m.isHidden(),
                o = [];
            for (Qa(a, this), n && this.cloneRenderTo(), this.layOutTitles(); l--;) if (a = c[l], a.options.stacking && (g = !0, a.isDirty)) {
                h = !0;
                break
            }
    ...
    Ea.prototype = {
        ...
        optionsToObject: function (a) {
            ...
            else if (La(a)) for (a.length > e && (c = typeof a[0], "string" === c ? b.name = a[0] : "number" === c && (b.x = a[0]), f++); e > g;) b[d[g++]] = a[f++];
    ...
    O.prototype = {
        ...
        updateParallelArrays: function (a, b) {
            var c = a.series,
                d = arguments;
            q(c.parallelArrays, "number" == typeof b ?
            function (d) {
            ...
            if (h = a.length, b = p(b, !0), d === !1 || !h || g !== h || e.cropped || e.hasGroupedData) {
                if (e.xIncrement = null, e.pointRange = m ? 1 : i.pointRange, e.colorCounter = 0, q(this.parallelArrays, function (a) {
                    ...
                    if (ia(k)) {
                        ...
                        e.xIncrement = m
                    } else if (La(k)) if (s) for (c = 0; h > c; c++) i = a[c], r[c] = i[0], u[c] = i.slice(1, s + 1);
                    else for (c = 0; h > c; c++) i = a[c], r[c] = i[0], u[c] = i[1];
                    else oa(12)
        ...
        generatePoints: function () {
            ...
            if (b && (g !== (c = b.length) || j)) for (m = 0; c > m; m++) m === h && !j && (m += g), b[m] && (b[m].destroyElements(), b[m].plotX = t);
        ...
        getExtremes: function (a) {
            ...
            for (d = a.length, l = 0; d > l; l++) if (j = c[l], k = a[l], i = null !== k && k !== t && (!b.isLog || k.length || k > 0), j = this.getExtremesFromAll || this.cropped || (c[l + 1] || j) >= g && (c[l - 1] || j) <= h, i && j) if (i = k.length) for (; i--;) null !== k[i] && (e[f++] = k[i]);
            else e[f++] = k;
        ...
        drawPoints: function () {
            ...
            if (d.enabled !== !1 || this._hasPointMarkers) for (f = b.length; f--;) g = b[f], d = U(g.plotX), e = g.plotY, k = g.graphic, i = g.marker || {}, a = o && i.enabled === t || i.enabled, m = c.isInsidePlot(v(d), e, c.inverted), a && e !== t && !isNaN(e) && null !== g.y ? (a = g.pointAttr[g.selected ? "select" : ""] || l, h = a.r, i = p(i.symbol, this.symbol), j = 0 === i.indexOf("url"), k ? k[m ? "show" : "hide"](!0).animate(r({
                x: d - h,
                y: e - h
            }, k.symbolName ? {
                width: 2 * h,
        ...
        getAttribs: function () {
            ...
            if (b.marker ? (e.radius = e.radius || c.radius + e.radiusPlus, e.lineWidth = e.lineWidth || c.lineWidth + e.lineWidthPlus) : e.color = e.color || ya(e.color || g).brighten(e.brightness).get(), j[""] = a.convertAttribs(c, f), q(["hover", "select"], function (b) {
                ...
                if (i = h[g], (c = i.options && i.options.marker || i.options) && c.enabled === !1 && (c.radius = 0), i.negative && m && (i.color = i.fillColor = m), k = b.colorByPoint || i.color, i.options) for (p in l) s(c[l[p]]) && (k = !0);
        ...
        addPoint: function (a, b, c, d) {
            ...
            if (Qa(d, i), c && q([g, h, this.graphNeg, this.areaNeg], function (a) {
                ...
                series: this
            }, this.pointClass.prototype.applyOptions.apply(d, [a]), g = d.x, h = n.length, this.requireSorting && g < n[h - 1]) for (m = !0; h && n[h - 1] > g;) h--;
    ...
    var qa = la(O, {
        ...
        getSegments: function () {
            ...
            if (this.options.stacking && !this.cropped) {
                ...
                d.sort(function (a, b) {
                    ...
                    var k, d = 0;
                    if (!l || h[b] && null !== h[b].y) if (h[b]) c.push(h[b]);
                    else {
        ...
        getSegmentPath: function (a) {
            ...
            if (3 === d && c.push("L", b[1], b[2]), e.stacking && !this.closedStacks) for (d = a.length - 1; d >= 0; d--) g = p(a[d].yBottom, f), d < a.length - 1 && e.step && c.push(a[d + 1].plotX, g), c.push(a[d].plotX, g);
    ...
    J.area = qa, ca.spline = w(T), ha = la(O, {
                ...
                if (L) {
                    ...
                    if (m > 0) {
                        ...
                        for (y = 0; L > y; y++) {
                            ...
                            if (y > G && null !== H[y]) G = y;
                            else for (L - y + G > w && null !== H[y] && (G = w - L + y); null === H[G];) G++;
                            M.push({
                                i: G,
                                y: H[G]
                            }), H[G] = null
        ...
        drawTrackerGraph: function () {
            ...
            if (e && !c) for (m = e + 1; m--;)"M" === d[m] && d.splice(m + 1, 0, d[m + 1] - i, d[m + 2], "L"), (m && "M" === d[m] || m === e) && d.splice(m, 0, "L", d[m - 2] + i, d[m - 1]);
        ...
        setTooltipPoints: function (a) {
            ...
            if (this.options.enableMouseTracking !== !1 && !this.singularTooltips) {
                for (a && (this.tooltipPoints = null), q(this.segments || this.points, function (a) {
                    b = b.concat(a)
                }), e && e.reversed && (b = b.reverse()), this.orderTooltipPoints && this.orderTooltipPoints(b), a = b.length, i = 0; a > i; i++) if (e = b[i], c = e.x, c >= f.min && c <= f.max) for (h = b[i + 1], c = d === t ? 0 : d + 1, d = b[i + 1] ? C(u(0, U((e.clientX + (h ? h.wrappedClientX || h.clientX : g)) / 2)), g) : g; c >= 0 && d >= c;) j[c++] = e;
...
}), angular.module("sample.widgets.github", ["adf.provider", "highcharts-ng"]).value("githubApiUrl", "https://api.github.com/repos/").config(["dashboardProvider", function (dashboardProvider) {
    var widget = {
