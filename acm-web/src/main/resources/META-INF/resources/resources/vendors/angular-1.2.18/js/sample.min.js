/*! angular-dashboard-framework 0.1.0 (2014-09-04) */
angular.module("ui.bootstrap", ["ui.bootstrap.transition", "ui.bootstrap.collapse", "ui.bootstrap.accordion", "ui.bootstrap.alert", "ui.bootstrap.bindHtml", "ui.bootstrap.buttons", "ui.bootstrap.carousel", "ui.bootstrap.dateparser", "ui.bootstrap.position", "ui.bootstrap.datepicker", "ui.bootstrap.dropdown", "ui.bootstrap.modal", "ui.bootstrap.pagination", "ui.bootstrap.tooltip", "ui.bootstrap.popover", "ui.bootstrap.progressbar", "ui.bootstrap.rating", "ui.bootstrap.tabs", "ui.bootstrap.timepicker", "ui.bootstrap.typeahead"]), angular.module("ui.bootstrap.transition", []).factory("$transition", ["$q", "$timeout", "$rootScope",
    ...
    }.call(this), angular.module("sample", ["adf", "sample.widgets.ecmscharts", "sample.widgets.myassigneditems", "LocalStorageModule", "structures", "sample-01", "sample-02", "ngRoute", "ngTable"]).config(["$routeProvider", "localStorageServiceProvider",
...
]), angular.module("sample.widgets.myassigneditems", ["adf.provider"]).config(["dashboardProvider",
    function(dashboardProvider) {
        dashboardProvider.widget("myassigneditems", {
            title: "My Assigned Items",
            description: "Displays the assigned items to the user",
            templateUrl: "scripts/widgets/myassigneditems/myassigneditems.html",
            controller: "myAssignedItemsCtrl",
            edit: {
                templateUrl: "scripts/widgets/myassigneditems/edit.html"
            }
        })
    }
]).controller("myAssignedItemsCtrl", ["$scope", "$filter", "$http", "ngTableParams",
    function($scope, $filter, $http, ngTableParams) {
        var url = App.Object.getContextPath() + "/api/v1/plugin/document/assignedToUser/" + App.Object.getUserName();
        $http.get(url).success(function(rawData) {
            dataI=_.map(rawData,function(row){
                row=_.clone(row),
                row.itemUrl = App.Object.getContextPath() + "/plugin/casefile/";
                return row
            })
            $scope.isAssignedItems = dataI.length > 0 ? !0 : !1;
                ...
                sorting: {
                    ...
                    getData: function($defer, params) {
                            orderedData = params.sorting() ? $filter("orderBy")(dataI, params.orderBy()) : dataI;
...
Showdown.converter = function(converter_options) {
    ...
    angular.module("sample.widgets.ecmscharts", ["adf.provider", "highcharts-ng"]).config(["dashboardProvider",
    function(dashboardProvider) {
        var widget = {
            templateUrl: "scripts/widgets/ecms/ecms.html",
            reload: !0,
            resolve: {
                cases: function(ecmsServiceCases) {
                    return ecmsServiceCases.get()
                }
            },
            edit: {
                templateUrl: "scripts/widgets/ecms/edit.html"
            }
        };
        dashboardProvider.widget("caseByStatus", angular.extend({
            title: "Case by Status",
            description: "Displays Case by Status as pie chart",
            controller: "caseByStatusCtrl"
        }, widget))
    }
]).service("ecmsServiceCases", ["$q", "$http",
    function($q, $http) {
        return {
            get: function() {
                var deferred = $q.defer(),
                    url = App.Object.getContextPath() + "/api/latest/plugin/casebystatus";
                return $http.get(url).success(function(data) {
...
]).controller("caseByStatusCtrl", ["$scope", "config", "cases",
    ...
    function($templateCache) {
        "use strict";
        $templateCache.put("scripts/widgets/ecms/ecms.html", '<div><div class="alert alert-info" ng-if="!showChartCase"><p style="text-align:center;">Currently, there are no outstanding cases</p></div><div ng-if="showChartCase"><highchart id="chart1" config="chartConfig"></highchart></div></div>'),
        $templateCache.put("scripts/widgets/ecms/edit.html", '<form role="form"><div class="form-group"></div></form>'),


        $templateCache.put("scripts/widgets/myassigneditems/edit.html", '<form role="form"><div class="form-group"></div></form>'),
        $templateCache.put("scripts/widgets/myassigneditems/myassigneditems.html",
            '<div class="myassigneditems">' +
                '<div class="alert alert-info" ng-if="!isAssignedItems">' +
                    '<p style="text-align:center;">No active items assigned</p>' +
                '</div>' +
                '<div ng-controller="myAssignedItemsCtrl" ng-if="isAssignedItems">' +
                    '<div style="overflow-x: auto;">'+
                        '<table ng-table="tableParams" class="table">' +
                            '<tr ng-repeat="itema in $data">' +
                                '<td data-title="\'ID\'" sortable="\'id\'">{{itema.id}}</td>' +
                //                              '<td data-title="\'ID\'" sortable="\'id\'"><a ng-href="{{itema.itemUrl}}{{itema.caseNumber}}">{{itema.id}}</a></td>' +
                                '<td data-title="\'Type\'" sortable="\'documentType\'">{{itema.documentType}}</td>' +
                                '<td data-title="\'Item Number\'" sortable="\'itemNumber\'">{{itema.itemNumber}}</td>' +
                                '<td data-title="\'Status\'" sortable="\'documentStatus\'">{{itema.documentStatus}}</td>' +
                                '<td data-title="\'Assignee\'" sortable="\'assignee\'">{{itema.assignee}}</td>' +
                                '<td data-title="\'Case Number\'" sortable="\'caseNumber\'">{{itema.caseNumber}}</td>' +
                            '</tr>' +
                        '</table>' +
                    '</div>' +
                '</div>' +
