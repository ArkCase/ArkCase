...
public class ChemistryCMISFacade implements CMISFacade
{
        ...
        if (session != null)
        {
            ...
            log.debug(
                    "Preparing to create a document with file name \"" + filename + "\" in folder \"" +
                            folderPath + "\".");
            returnId = createDocument(force ? getOrCreateFolderByPath(folderPath) : session.getObjectByPath(folderPath),
                    filename,
                    content,
                    mimeType,
                    versioningState,
            ...
            String relationshipType)
    {
        ...
        try
        {
            ...
            if (parentObj == null)
            {
                log.error("The parent object with ID \"" + parentObjectId
                        + "\" doesn't exists in the repository. No relationship will be created.");
                return null;
            }
        }
        catch (Exception objEx)
        {
            log.error("An error occurred while attempting to determine if an the parent object with ID \"" + parentObjectId
                    + "\" exists in the repository. " + objEx.getMessage());
            return null;
        }
        ...
        try
        {
            ...
            if (childObj == null)
            {
                log.error("The child object with ID \"" + childObjectId
                        + "\" doesn't exists in the repository. No relationship will be created.");
                return null;
            }
        }
        ...
        {
            log.error("An error occurred while attempting to determine if an the child object with ID \"" + childObjectId
                    + "\" exists in the repository. " + objEx.getMessage());
            return null;
        }

        // Set-up the paramters in preparation for the "createRelationship" call.
        Map<String, Serializable> relProps = new HashMap<>();
        ...
        {
            log.error("An error occurred while attempting to create a relationship between the parent object with ID \"" + parentObjectId
                    + "\" and the child object with ID \"" + childObjectId + "\". " + relEx.getMessage());
            return null;
        }
    }
            ...
            while (keySetItr.hasNext())
            {
                ...
                if (currentVal != null)
                {
                    ...
                    if (currentKey.toLowerCase().startsWith("m:"))
                    {
                        ...
                        ArrayList<String> propArray = new ArrayList<>(Arrays.asList(valArray));
