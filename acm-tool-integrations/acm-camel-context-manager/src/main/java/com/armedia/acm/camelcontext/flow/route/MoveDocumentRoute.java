 ...
 */
public class MoveDocumentRoute extends RouteBuilder implements ArkCaseRoute
{
    ...
    public void configure()
    {
        onException(Exception.class).handled(false)
                .process(x -> {
                    ...
                    log.error("Exception moving document: {}", causeMessage, exception);
                    throw new ArkCaseFileRepositoryException(exception);
                });

        from("seda:" + repositoryId + "-moveDocumentQueue?timeout=" + timeout).setExchangePattern(ExchangePattern.InOut)
                .process(exchange -> {
                    ...
                    exchange.getIn().getHeaders().put(CamelCMISConstants.CMIS_OBJECT_ID, map.get("cmisObjectId"));
                    exchange.getIn().getHeaders().put(CamelCMISConstants.CMIS_SOURCE_FOLDER_ID, map.get("srcFolderId"));
