 ...
 */
public class GetObjectByPathRoute extends ArkCaseAbstractRoute
{
    ...
    public void configure()
    {
        onException(Exception.class).handled(false)
                .process(x -> {
                    ...
                    log.error("Exception while finding object by path: {}", causeMessage, exception);
                    throw new ArkCaseFileRepositoryException(exception);
                });

        from("seda:" + getRepositoryId() + "-getObjectByPathQueue?timeout=" + getTimeout()).setExchangePattern(ExchangePattern.InOut)
                .process(exchange -> {
