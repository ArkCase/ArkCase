 ...
 */
public class GetObjectByIdRoute extends RouteBuilder implements ArkCaseRoute
{
    ...
    public void configure() throws Exception
    {
        onException(Exception.class).handled(false)
                .process(x -> {
                    ...
                    log.error("Exception while finding object by id: {}", causeMessage, exception);
                    throw new ArkCaseFileRepositoryException(exception);
                });

        from("seda:" + repositoryId + "-getObjectByIdQueue?timeout=" + timeout).setExchangePattern(ExchangePattern.InOut)
                .process(exchange -> {
