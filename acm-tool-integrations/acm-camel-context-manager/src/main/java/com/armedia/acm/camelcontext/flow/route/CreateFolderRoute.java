 ...
 */
public class CreateFolderRoute extends ArkCaseAbstractRoute
{
    ...
    public void configure()
    {
        onException(Exception.class).handled(false)
                .process(x -> {
                    ...
                    log.error("Exception creating new folder: {}", causeMessage, exception);
                    throw new ArkCaseFileRepositoryException(exception);
                });

        from("seda:" + getRepositoryId() + "-createFolderQueue?timeout=" + getTimeout()).setExchangePattern(ExchangePattern.InOut)
                .process(exchange -> {
                    ...
                    exchange.getIn().getHeaders().put(PropertyIds.NAME, routeProperties.get("newFolderName"));
                    exchange.getIn().getHeaders().put(CamelCMISConstants.CMIS_OBJECT_ID, routeProperties.get("parentFolderId"));
