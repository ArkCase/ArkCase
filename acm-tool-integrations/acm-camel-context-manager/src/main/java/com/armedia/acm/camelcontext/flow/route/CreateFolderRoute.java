 ...
 */
public class CreateFolderRoute extends RouteBuilder implements ArkCaseRoute
{
    ...
    public void configure()
    {
        ...
        from("seda:" + repositoryId + "-createFolderQueue?timeout=" + timeout).setExchangePattern(ExchangePattern.InOut)
                .process(exchange -> {
                    ...
                    exchange.getIn().getHeaders().put(PropertyIds.NAME, map.get("newFolderName"));
                    exchange.getIn().getHeaders().put(CamelCMISConstants.CMIS_OBJECT_ID, map.get("parentFolderId"));
