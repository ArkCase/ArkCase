 ...
 */
public class RenameDocumentRoute extends ArkCaseAbstractRoute
{
    ...
    public void configure()
    {
        onException(Exception.class).handled(false)
                .process(x -> {
                    ...
                    log.error("Exception renaming document: {}", causeMessage, exception);
                    throw new ArkCaseFileRepositoryException(exception);
                });

        from("seda:" + getRepositoryId() + "-renameDocumentQueue?timeout=" + getTimeout()).setExchangePattern(ExchangePattern.InOut)
                .process(exchange -> {
                    ...
                    exchange.getIn().getHeaders().put(PropertyIds.NAME, FileCamelUtils.replaceSurrogateCharacters(String.valueOf(routeProperties.get(ArkCaseCMISConstants.NEW_FILE_NAME)), 'X'));
