 ...
 */
public class ListFolderRoute extends ArkCaseAbstractRoute
{
    ...
    public void configure()
    {
        onException(Exception.class).handled(true)
                .process(x -> {
                    ...
                    log.error("Exception listing: {}", causeMessage, exception);
                    throw new ArkCaseFileRepositoryException(exception);
                });

        from("seda:" + getRepositoryId() + "-listFolderQueue?timeout=" + getTimeout()).setExchangePattern(ExchangePattern.InOut)
                .process(exchange -> {
                    ...
                    exchange.getIn().getHeaders().put(CamelCMISConstants.CMIS_ACTION, CamelCMISActions.LIST_FOLDER);
