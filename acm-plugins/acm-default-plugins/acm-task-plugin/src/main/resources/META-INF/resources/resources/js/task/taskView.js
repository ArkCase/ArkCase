...
//            this.setFormUrls(formUrls);
            ...
            this.$chkRestrict.on("click", function(e) {Task.View.Detail.onClickRestrictCheckbox(e, this);});

            // Reject Task
            this.$dlgRejectTask = $('#reject');
            this.$btnRejectTask = $("#btnReject");
            this.$btnRejectTask.click(function(e) {Task.View.Detail.onClickBtnRejectTask(e,this);});
            ...
            this.$btnSearchRejectTask.click(function(e) {Task.View.Detail.onClickSearchRejectTask(e,this);});
            this.$inputSearchRejectTask = this.$dlgRejectTask.find("input[name=searchKeywordRejectTask]");
            this.$inputSearchRejectTask.keyup(function(e) {Task.View.Detail.onKeyUpSearchRejectTask(e,this);});
            this.$txtCommentRejectTask = this.$dlgRejectTask.find("textarea[id=commentRejectTask]");
            this.$txtCommentRejectTask.change(function(e) {Task.View.Detail.onChangeCommentRejectTask(e,this);});
            ...
            this.$dlgRejectTaskSortableColumns.each(function(index) {
                $(this).click(function(e) {Task.View.Detail.onClickDlgRejectTaskSortableColumn(e,this);});
            });
...
//        }
        ...
        ,initDlgRejectTask: function() {
            Task.Model.Detail.cleanDlgRejectTaskOwner(this.$dlgRejectTask);
            Task.Model.Detail.cleanDlgRejectTaskUsers(this.$dlgRejectTask);
        ...
        ,_refreshDlgRejectTaskOwner: function(tbody, data) {
            ...
            if (tbody && data && data.response && data.response.docs && data.response.docs.length > 0) {
                Task.Model.Detail.buildDlgRejectTaskOwner(tbody, data.response.docs);
            }
        }
        ,_refreshDlgRejectTaskUsers: function(tbody, data) {
            if (tbody && data && data.response && data.response.docs && data.response.docs.length > 0) {
                Task.Model.Detail.buildDlgRejectTaskUsers(tbody, data.response.docs);
            }
        }
        ,_refreshDlgRejectTaskPaging: function(data) {
            ...
            Task.Model.Detail.buildDlgRejectTaskMutedText($textMuted, from, to, total);

            // Build Pagintion
            $ulPagination = this.$dlgRejectTask.find('footer.panel-footer ul.pagination');
            Task.Model.Detail.buildDlgRejectTaskPagination($ulPagination, page, pages);

            this.setDlgRejectTaskStart(data.response.start);
        }
        // Reject Task Events
        ,onModelRetrievedUsers: function(users){
            Task.View.Detail.refreshDlgRejectTaskUsers(users);
        }
        ,_onRetrieveUsers: function() {
            var task = Task.View.getActiveTask();
            var start = Task.View.Detail.getDlgRejectTaskStart();
            var n = Task.View.Detail.getDlgRejectTaskN();
            var sortDirection = Task.View.Detail.getDlgRejectTaskSortDirection();
            var searchKeyword = Task.View.Detail.getDlgRejectTaskSearchKeyword();
        ...
        ,onClickBtnRejectTask: function(event,ctrl) {
            Task.View.Detail.initDlgRejectTask();
            this._onRetrieveUsers();
            Task.View.Detail.showDlgRejectTask(function(event, ctrl) {
                var returnTo = Task.View.Detail.getDlgRejectTaskSelected();
                var note = Task.View.Detail.getDlgRejectTaskComment();
                if (returnTo != null) {
                    Task.View.Detail.onSaveAssignee(returnTo);
                    if (note && note.trim() !== '') {
                        var noteToSave = {};
        ...
        ,onClickDlgRejectTaskSortableColumn: function(event,ctrl) {
            ...
            if (sortDirection && sortDirection == 'ASC') {
                Task.View.Detail.setDlgRejectTaskSortDirection('DESC');
            } else {
                Task.View.Detail.setDlgRejectTaskSortDirection('ASC');
            }
        ...
        ,onClickDlgRejectTaskLeftBtn: function(event,ctrl) {
            var start = Task.View.Detail.getDlgRejectTaskStart();
            var page = Task.View.Detail.getDlgRejectTaskPage();

            if (page > 0) {
                Task.View.Detail.setDlgRejectTaskStart(start - Task.View.Detail.getDlgRejectTaskN());
                Task.View.Detail.setDlgRejectTaskPage(page - 1);
            }
        ...
        ,onClickDlgRejectTaskRightBtn: function(event,ctrl) {
            var start = Task.View.Detail.getDlgRejectTaskStart();
            var page = Task.View.Detail.getDlgRejectTaskPage();
            var pages = Task.View.Detail.getDlgRejectTaskPages();
        ...
        ,onClickDlgRejectTaskPageBtn: function(event,ctrl) {
            ...
            if (page) {
                Task.View.Detail.setDlgRejectTaskStart((page - 1) * Task.View.Detail.getDlgRejectTaskN());
                Task.View.Detail.setDlgRejectTaskPage(page);
            }

            this._onRetrieveUsers();
        }
        ,onClickSearchRejectTask: function(event,ctrl) {
            var keyword = Task.View.Detail.$inputSearchRejectTask.val();
            Task.View.Detail.setDlgRejectTaskSearchKeyword(keyword);

            Task.View.Detail.setDlgRejectTaskStart(Task.DLG_REJECT_TASK_START);
            Task.View.Detail.setDlgRejectTaskN(Task.DLG_REJECT_TASK_N);
            Task.View.Detail.setDlgRejectTaskSortDirection(Task.DLG_REJECT_TASK_SORT_DIRECTION);
            Task.View.Detail.setDlgRejectTaskPage(0);
            Task.View.Detail.setDlgRejectTaskPages(0);
        ...
        ,onChangeCommentRejectTask: function(event,ctrl) {
            var comment = Task.View.Detail.$txtCommentRejectTask.val();
            if (comment && comment.trim() !== '') {
                Task.View.Detail.setDlgRejectTaskComment(comment);
            }
        }
        ,onChangeDlgRejectTaskSelected: function(event,ctrl) {
            Task.View.Detail.setDlgRejectTaskSelected($(event.target).val());
        }
        // end of Reject ------------------------------------------
...
//        }
        ...
        ,onModelSavedReworkDetails: function(details){
            if (details.hasError) {
