...
var _initDashboard = function () {
    ...
    angular.module("ui.bootstrap", ["ui.bootstrap.transition", "ui.bootstrap.collapse", "ui.bootstrap.accordion", "ui.bootstrap.alert", "ui.bootstrap.bindHtml", "ui.bootstrap.buttons", "ui.bootstrap.carousel", "ui.bootstrap.dateparser", "ui.bootstrap.position", "ui.bootstrap.datepicker", "ui.bootstrap.dropdown", "ui.bootstrap.modal", "ui.bootstrap.pagination", "ui.bootstrap.tooltip", "ui.bootstrap.popover", "ui.bootstrap.progressbar", "ui.bootstrap.rating", "ui.bootstrap.tabs", "ui.bootstrap.timepicker", "ui.bootstrap.typeahead"]), angular.module("ui.bootstrap.transition", []).factory("$transition", ["$q", "$timeout", "$rootScope",
        ...
        }.call(this), angular.module("sample", ["adf", "sample.widgets.casesummary", "sample.widgets.mycases", "sample.widgets.mytasks", "sample.widgets.mycomplaints", "sample.widgets.news", "sample.widgets.weather", "sample.widgets.teamtaskworkload", "sample.widgets.activecasefilesbyqueue", "LocalStorageModule", "structures", "sample-01", "sample-02", "ngRoute", "ngTable"]).config(["$routeProvider", "localStorageServiceProvider",
        ...
        function (dashboardProvider) {
            dashboardProvider.widget("news", {
                title: $.t("dashboard:plugins.news.title"),
                description: $.t("dashboard:plugins.news.label.description"),
        ...
        function (dashboardProvider) {
            dashboardProvider.widget("myTasks", {
                title: $.t("dashboard:plugins.my-tasks.title"),
                description: $.t("dashboard:plugins.my-tasks.label.description"),
...
//    }
        ...
        function (dashboardProvider) {
            dashboardProvider.widget("myCases", {
                title: $.t("dashboard:plugins.my-cases.title"),
                description: $.t("dashboard:plugins.my-cases.label.description"),
        ...
        angular.module("sample.widgets.mycomplaints", ["adf.provider"]).config(["dashboardProvider",
            function (dashboardProvider) {
                dashboardProvider.widget("myComplaints", {
                    title: $.t("dashboard:plugins.my-complaints.title"),
                    description: $.t("dashboard:plugins.my-complaints.label.description"),
                ...
                $http.get(url).success(function (rawData) {
                    ...
                    $scope.tableParams = new ngTableParams({
                        ...
                        getData: function ($defer, params) {
                            ...
                            var orderedData = params.sorting() ? $filter("orderBy")(dataC, params.orderBy()) : dataC;
                            params.total(orderedData.length), $defer.resolve(orderedData.slice((params.page() - 1) * params.count(), params.page() * params.count()))
                        }
                    })
                })
        ...
        "http://api.openweathermap.org/data/2.5/weather?units=metric&callback=JSON_CALLBACK&q=").config(["dashboardProvider",
            function (dashboardProvider) {
                dashboardProvider.widget("weather", {
                    title: $.t('dashboard:plugins.weather.title'),
                    description: $.t('dashboard:plugins.weather.label.description'),
        ...
        function (dashboardProvider) {
            dashboardProvider.widget("linklist", {
                title: $.t("dashboard:plugins.links-list.title"),
                description: $.t("dashboard:plugins.links-list.label.description"),
    ...
    Showdown.converter = function (converter_options) {
        ...
        function () {
            ...
            ga && (S.CanVGRenderer = Z = function () {
                ...
                init: function (a, b) {
                    ...
                    -1 === Da(this, a.axes) && (c && !this.isColorAxis ? a.axes.splice(a.xAxis.length, 0, this) : a.axes.push(this), a[this.coll].push(this)), this.series = this.series || [], a.inverted && c && this.reversed === t && (this.reversed = !0), this.removePlotLine = this.removePlotBand = this.removePlotBandOrLine;
        ...
        function (dashboardProvider) {
            ...
            dashboardProvider.widget("casesByStatusSummary", angular.extend({
                title: $.t('dashboard:plugins.casessum.title'),
                description: $.t('dashboard:plugins.casessum.label.description'),
        ...
        function (dashboardProvider) {
            ...
            dashboardProvider.widget("teamTaskWorkload", angular.extend({
                title: $.t('dashboard:plugins.team-task-workload.title'),
                description: $.t('dashboard:plugins.team-task-workload.label.description'),
                controller: "teamTaskWorkloadCtrl",
                text: $.t('dashboard:plugins.team-task-workload.label.no-outstanding-tasks')
            }, widget)).widget("newComplaints", angular.extend({
                title: $.t('dashboard:plugins.new-complaints.title'),
                description: $.t('dashboard:plugins.new-complaints.label.description'),
                controller: "newComplaintsCtrl",
                text: $.t('dashboard:plugins.new-complaints.label.no-new-complaints')
            }, widget))
        }
        ...
        function ($scope, config, complaints) {
            $scope.text = $.t('dashboard:plugins.new-complaints.label.no-new-complaints');
            ...
            angular.forEach(data, function (count, day) {
                ...
                xAxis: {
                    type: "datetime",
                    title: {
                        text: $.t('dashboard:plugins.new-complaints.label.chart-date')
                    }
                },
                yAxis: {
                    title: {
                        text: $.t('dashboard:plugins.new-complaints.label.chart-complaints')
                    },
                    min: 0
                },
                series: [{
        ...
        function ($scope, config, tasks) {
            $scope.text = $.t('dashboard:plugins.team-task-workload.label.no-outstanding-tasks');
            $scope.showChart = tasks.length > 0 ? true : false;

            var options = [
                {
                    idO: "all", nameO: $.t('dashboard:plugins.team-task-workload.label.all')
                },
                {
                    idO: "pastDue", nameO: $.t('dashboard:plugins.team-task-workload.label.past-due')
                },
                {
                    idO: "dueTomorrow", nameO: $.t('dashboard:plugins.team-task-workload.label.due-tomorrow')
                },
                {
                    idO: "dueInAWeek", nameO: $.t('dashboard:plugins.team-task-workload.label.due-7-days')
                },
                {
                    idO: "dueInAMonth", nameO: $.t('dashboard:plugins.team-task-workload.label.due-30-days')
                }];

            var chartTitle = $.t('dashboard:plugins.team-task-workload.label.all');
            angular.forEach(options, function (option) {
    ...
    ]),angular.module("sample.widgets.activecasefilesbyqueue", ["adf.provider", "highcharts-ng"]).config(["dashboardProvider",
            function (dashboardProvider) {
                var widget = {

                    templateUrl: "scripts/widgets/activecasefilesbyqueue/activecasefilesbyqueue.html",
                    reload: !0,
                    resolve: {
                        caseFilesByQueue: function (activeCaseFilesByQueueService, config) {
                            return activeCaseFilesByQueueService.getActiveCaseFilesByQueue()
                        }
                    }
                };
                dashboardProvider.widget("activeCaseFilesByQueue", angular.extend({
                    title: $.t('dashboard:plugins.active-casefiles-by-queue.title'),
                    description: $.t('dashboard:plugins.active-casefiles-by-queue.label.description'),
                    controller: "activeCaseFilesByQueueController",
                    text: $.t('dashboard:plugins.active-casefiles-by-queue.title')
                }, widget))
            }
        ]).service("activeCaseFilesByQueueService", ["$q", "$http",
            function ($q, $http) {
                return {
                    getActiveCaseFilesByQueue: function () {
                        var deferred = $q.defer(),
        ...
        ]).controller("activeCaseFilesByQueueController", ["$scope", "config", "caseFilesByQueue",
            function ($scope, config, caseFilesByQueue) {
                $scope.text = $.t('dashboard:plugins.active-casefiles-by-queue.label.no-data')
                $scope.showChart = angular.equals(null, caseFilesByQueue) || angular.equals({}, caseFilesByQueue) ? false : true;

                var data = [];

                angular.forEach(caseFilesByQueue, function(value, key){
                    var item = {};
                    item.name = key;
                    item.y = value;
                    item.drilldown = key;
                ...
                caseFilesByQueue && ($scope.chartConfig = {
                    chart: {
                        type: 'column'
                    },
                    title: {
                    ...
                    xAxis: {
                        type: 'category',
                        title: {
                            text: $.t('dashboard:plugins.active-casefiles-by-queue.label.xAxis-title')
                        }
                    },
                    yAxis: {
                        title: {
                            text: $.t('dashboard:plugins.active-casefiles-by-queue.label.yAxis-title')
                        }
                    },
                    series: [{
                        type: 'column',
                        ...
                        name: $.t('dashboard:plugins.active-casefiles-by-queue.title'),
                        data: data
                    }]
                })
            }
        ...
        function ($templateCache) {
            ...
            $templateCache.put("../src/templates/dashboard-edit.html",
                ...
                '<h4 class="modal-title">' + $.t('dashboard:label.edit-dashboard') + '</h4>' +
                '</div>' +
                '<div class="modal-body">' +
                '<form role="form">' +
                '<div class="form-group">' +
                '<label for="dashboardTitle">' + $.t('dashboard:label.title') + '</label>' +
                '<input type="text" class="form-control" id="dashboardTitle" ng-model="model.title" required>' +
                '</div>' +
                '<div class="form-group"><label>' + $.t('dashboard:label.structure') + '</label>' +
                ...
                '<button type="button" class="btn btn-primary" ng-click="closeDialog()">' + $.t('dashboard:label.close') + '</button>' +
                '</div>'),
                ...
                $templateCache.put("../src/templates/widget-add.html", '<div class="modal-header"><button type="button" class="close" ng-click="closeDialog()" aria-hidden="true">&times;</button><h4 class="modal-title">'+ $.t('dashboard:label.add-new-widget') +'</h4></div><div class="modal-body"><div style="display: inline-block"><dl class="dl-horizontal"><dt ng-repeat-start="(key, widget) in widgets"><a href="" ng-click="addWidget(key)">{{widget.title}}</a></dt><dd ng-repeat-end="" ng-if="widget.description">{{widget.description}}</dd></dl></div></div><div class="modal-footer"><button type="button" class="btn btn-primary" ng-click="closeDialog()">'+ $.t('dashboard:label.close') +'</button></div>'),
                $templateCache.put("../src/templates/widget-edit.html", '<div class="modal-header"><button type="button" class="close" ng-click="closeDialog()" aria-hidden="true">&times;</button><h4 class="modal-title">{{widget.title}}</h4></div><div class="modal-body"><form role="form"><div class="form-group"><label for="widgetTitle">'+ $.t('dashboard:label.title') +'</label><input type="text" class="form-control" id="widgetTitle" ng-model="definition.title" placeholder="'+ $.t('dashboard:label.enter-title') +'" required></div></form><div ng-if="widget.edit"><adf-widget-content model="definition" content="widget.edit"></div></div><div class="modal-footer"><button type="button" class="btn btn-primary" ng-click="closeDialog()">'+ $.t('dashboard:label.close') +'</button></div>'),
        ...
        function ($templateCache) {
            "use strict";
            $templateCache.put("scripts/widgets/github/edit.html", '<form role="form"><div class="form-group"><label for="path">Github Repository Path</label><input type="text" class="form-control" id="path" ng-model="config.path" placeholder="Enter Path (username/reponame)"></div></form>'),
                $templateCache.put("scripts/widgets/github/github.html", '<div><div class="alert alert-info" ng-if="!chartConfig">Please insert a repository path in the widget configuration</div><div ng-if="chartConfig"><highchart id="chart1" config="chartConfig"></highchart></div></div>'),
                $templateCache.put("scripts/widgets/linklist/edit.html", '<form class="form-inline" role="form"><div><label>'+ $.t('dashboard:plugins.links-list.label.links') +'</label></div><div class="padding-bottom" ng-repeat="link in config.links"><div class="form-group"><label class="sr-only" for="title-{{$index}}">'+ $.t('dashboard:plugins.links-list.label.title') +'</label><input type="text" id="title-{{$index}}" class="form-control" placeholder="Title" ng-model="link.title" required></div><div class="form-group"><label class="sr-only" for="href-{{$index}}">'+ $.t('dashboard:plugins.links-list.label.url') +'</label><input type="url" id="href-{{$index}}" class="form-control" placeholder="http://example.com" ng-model="link.href" required></div><button type="button" class="btn btn-warning" ng-click="removeLink($index)"><i class="fa fa-minus"></i> '+ $.t('dashboard:plugins.links-list.label.remove') +'</button></div><button type="button" class="btn btn-primary" ng-click="addLink()"><i class="fa fa-plus"></i> '+ $.t('dashboard:plugins.links-list.label.add') +'</button></form>'),
                $templateCache.put("scripts/widgets/linklist/linklist.html", '<div class="linklist"><ul><li ng-repeat="link in links | orderBy:\'title\'"><a target="_blank" ng-href="{{link.href}}">{{link.title}}</a></li></ul></div>'),
                $templateCache.put("scripts/widgets/markdown/edit.html", '<form role="form"><div class="form-group"><label for="content">Markdown content</label><textarea id="content" class="form-control" rows="5" ng-model="config.content"></textarea></div></form>'),
                ...
                $templateCache.put("scripts/widgets/mycomplaints/mycomplaints.html", '<div class="mycomplaints"><div class="alert alert-info" ng-controller="myComplaintsCtrl" ng-if="!isDataC"><p style="text-align:center;">'+ $.t('dashboard:plugins.new-complaints.label.no-new-complaints-to-you') +'</p></div><div ng-controller="myComplaintsCtrl" ng-if="isDataC"><div style="overflow-x: auto;">' +
...
//<label for="url">Feed url</label><input type="url" class="form-control" id="url" ng-model="config.url" placeholder="Enter feed url">
                ...
                $templateCache.put("scripts/widgets/mytasks/edit.html", '<form role="form"><div class="form-group" ><input type="text" class="form-control" id="rowsT" ng-model="config.rowsT" placeholder="' + $.t('dashboard:plugins.my-tasks.label.enter-row-numbers') + '"></div></form>'),
                $templateCache.put("scripts/widgets/mytasks/mytasks.html", '<div class="myTasks"><div class="alert alert-info"  ng-controller="myTasksCtrl" ng-if="!isData"><p style="text-align:center;">' + $.t('dashboard:plugins.my-tasks.label.no-tasks-assigned') + '</p></div><div ng-controller="myTasksCtrl" ng-if="isData"><div style="overflow-x: auto;"><table  ng-table="tableParams" class="table"><tr ng-repeat="task in $data"><td data-title="\'' + $.t('dashboard:plugins.my-tasks.label.task-id') + '\'" sortable="\'id\'"><a ng-href="{{task.taskUrl}}{{task.taskId}}">{{task.taskId}}</td><td data-title="\'' + $.t('dashboard:plugins.my-tasks.label.title') + '\'" sortable="\'title\'"><a ng-href="{{task.taskUrl}}{{task.taskId}}">{{task.title}}</td><td data-title="\'' + $.t('dashboard:plugins.my-tasks.label.priority') + '\'" sortable="\'priority\'">{{task.priority}}</td><td data-title="\'' + $.t('dashboard:plugins.my-tasks.label.due') + '\'" sortable="\'due\'">{{task.due}}</td><td data-title="\'' + $.t('dashboard:plugins.my-tasks.label.status') + '\'" sortable="\'status\'">{{task.status}}</td><td data-title="\'' + $.t('dashboard:plugins.my-tasks.label.case-id') + '\'" sortable="\'parentNumber\'"><a ng-href="{{task.parentObjectUrl}}{{task.parentID}}"><a ng-href="{{task.parentObjectUrl}}{{task.parentID}}">{{task.parentNumber}}</td></tr></table></div></div></div>'),
//<label for="url">Feed url</label><input type="url" class="form-control" id="url" ng-model="config.url" placeholder="Enter feed url">
                ...
                '<p style="text-align:center;">' + $.t('dashboard:plugins.my-cases.label.no-tasks-assigned') + '</p>' +
                '</div>' +
                '<div ng-controller="myCasesCtrl" ng-if="isData">' +
                '<div style="overflow-x: auto;">' +
                '<table  ng-table="tableParams" class="table">' +
                '<tr ng-repeat="case in $data">' +
                '<td data-title="\'' + $.t('dashboard:plugins.my-cases.label.number') + '\'" sortable="\'number\'"><a ng-href="{{case.caseUrl}}{{case.id}}">{{case.number}}</td>' +
                '<td data-title="\'' + $.t('dashboard:plugins.my-cases.label.title') + '\'" sortable="\'title\'"><a ng-href="{{case.caseUrl}}{{case.id}}">{{case.title}}</td>' +
                '<td data-title="\'' + $.t('dashboard:plugins.my-cases.label.priority') + '\'" sortable="\'priority\'">{{case.priority}}</td>' +
                '<td data-title="\'' + $.t('dashboard:plugins.my-cases.label.status') + '\'" sortable="\'status\'">{{case.status}}</td>' +
                '</tr>' +
                '</table>' +
                '</div>' +
                '</div>' +
                ...
                $templateCache.put("scripts/widgets/news/edit.html", '<form role="form"><div class="form-group"><label for="url">' + $.t("dashboard:plugins.news.label.feed-url") + '</label><input type="url" class="form-control" id="url" ng-model="config.url" placeholder="' + $.t("dashboard:plugins.news.label.enter-feed-url") + '"></div></form>'),
                $templateCache.put("scripts/widgets/news/news.html", '<div class="news"><div class="alert alert-info" ng-if="!feed">' + $.t("dashboard:plugins.news.label.insert-url") + '</div><h4><a ng-href="{{feed.link}}" target="_blank">{{feed.title}}</a></h4><ul><li ng-repeat="entry in feed.entries"><a ng-href="{{entry.link}}" target="_blank">{{entry.title}}</a></li></ul></div>'),
                ...
                '<label for="path">' + $.t('dashboard:plugins.casessum.label.select-time-period') + '</label>' +
                '<select type="text" class="form-control" id="period" ng-model="config.period">' +
                '<option value="all" ng-selected="selected">' + $.t('dashboard:plugins.casessum.label.all') + '</option>' +
                '<option value="lastWeek">' + $.t('dashboard:plugins.casessum.label.last-week') + '</option>' +
                '<option value="lastMonth">' + $.t('dashboard:plugins.casessum.label.last-month') + '</option>' +
                '<option value="lastYear">' + $.t('dashboard:plugins.casessum.label.last-year') + '</option>' +
                '</select>' +
                '</div></form>'),


                $templateCache.put("scripts/widgets/weather/edit.html", '<form role="form"><div class="form-group"><label for="location">'+ $.t('dashboard:plugins.weather.label.location') +'</label><input type="location" class="form-control" id="location" ng-model="config.location" placeholder="'+ $.t('dashboard:plugins.weather.label.enter-location') +'"></div></form>'),
                $templateCache.put("scripts/widgets/weather/weather.html", '<div class="text-center"><div class="alert alert-info" ng-if="!weather"><p style="text-align:center;">' + $.t('dashboard:plugins.weather.label.insert-location-in-configuration') + '</div>' +
                ...
                '<h3>' + $.t('dashboard:plugins.weather.label.current') + ' {{ weather.temp.current | temp:2 }}</h3>' + //<h3>Current: {{ weather.temp.current | temp:2 }} F</h3>
                'min: {{ weather.temp.min | temp:2 }}, max: {{ weather.temp.max | temp:2 }}' +
                '</div></div>'),

                $templateCache.put("partials/sample.html", '<adf-dashboard name="{{name}}" structure="4-8" adf-model="model" adf-widget-filter="widgetFilter"/>')
        }
    ])
