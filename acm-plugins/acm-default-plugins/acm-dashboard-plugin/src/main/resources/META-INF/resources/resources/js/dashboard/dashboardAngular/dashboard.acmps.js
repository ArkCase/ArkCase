    ...
    function($q, $http) {
        return {
            getMilestones: function() {
                var deferred = $q.defer(),
                    url = App.getContextPath() + "/api/latest/milestonebyname/CASE_FILE";
                return $http.get(url).success(function(data) {
...
//                        format: "<b>Count</b>: {point.percentage:.1f} %"
    ...
    function(dashboardProvider) {
        ...
        dashboardProvider.widget("teamTaskWorkload", angular.extend({
            ...
            text: "Currently, there are no outstanding tasks for any user"
            ...
            text: "Currently, there are no new complaints for any user"
        }, widget))
    }
    ...
    function($scope, config, complaints) {
        $scope.text = "Currently, there are no new complaints for any user";
    ...
    function($scope, config, tasks) {
        $scope.text = "Currently, there are no outstanding tasks for any user";
                        ...
                        '<div style="overflow-x: auto;">' +
                            '<table  ng-table="tableParams" class="table">' +
                                '<tr ng-repeat="case in $data">' +
                                    '<td data-title="\'Number\'" sortable="\'number\'"><a ng-href="{{case.caseUrl}}{{case.id}}">{{case.number}}</td>' +
                                    '<td data-title="\'Title\'" sortable="\'title\'"><a ng-href="{{case.caseUrl}}{{case.id}}">{{case.title}}</td>' +
