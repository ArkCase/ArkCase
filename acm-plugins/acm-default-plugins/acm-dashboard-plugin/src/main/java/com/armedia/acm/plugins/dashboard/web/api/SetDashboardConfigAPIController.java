...
public class SetDashboardConfigAPIController
{
    ...
    ) throws AcmObjectNotFoundException, AcmUserActionFailedException
    {
        ...
        try
        {
            ...
            d = dashboardDao.getDashboardConfigForUserAndModuleName(user, ModuleName.getModuleName(moduleName));
            int retval = getDashboardDao().setDasboardConfigForUserAndModule(user, updateDashboardDto, ModuleName.getModuleName(moduleName));
            if (retval != 1)
            {
