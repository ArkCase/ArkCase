...
Complaint.Model = Complaint.Model || {
    create: function () {
        if (Complaint.Service.create) {
            Complaint.Service.create();
        }
        if (Complaint.Model.Lookup.create) {
            Complaint.Model.Lookup.create();
        }
        if (Complaint.Model.Tree.create) {
            Complaint.Model.Tree.create();
        }
...
//        if (Complaint.Model.Action.create)                {Complaint.Model.Action.create();}
        if (Complaint.Model.Detail.create) {
            Complaint.Model.Detail.create();
        }
        if (Complaint.Model.People.create) {
            Complaint.Model.People.create();
        }
        if (Complaint.Model.Documents.create) {
            Complaint.Model.Documents.create();
        }
        if (Complaint.Model.Notes.create) {
            Complaint.Model.Notes.create();
        }
        if (Complaint.Model.References.create) {
            Complaint.Model.References.create();
        }
        if (Complaint.Model.Tasks.create) {
            Complaint.Model.Tasks.create();
        }
        if (Complaint.Model.Location.create) {
            Complaint.Model.Location.create();
        }
        if (Complaint.Model.History.create) {
            Complaint.Model.History.create();
        }
        if (Complaint.Model.Time.create) {
            Complaint.Model.Time.create();
        }
        if (Complaint.Model.Cost.create) {
            Complaint.Model.Cost.create();
        }
    }
    , onInitialized: function () {
        if (Complaint.Service.onInitialized) {
            Complaint.Service.onInitialized();
        }
        if (Complaint.Model.Lookup.onInitialized) {
            Complaint.Model.Lookup.onInitialized();
        }
        if (Complaint.Model.Tree.onInitialized) {
            Complaint.Model.Tree.onInitialized();
        }
...
//        if (Complaint.Model.Action.onInitialized)         {Complaint.Model.Action.onInitialized();}
        if (Complaint.Model.Detail.onInitialized) {
            Complaint.Model.Detail.onInitialized();
        }
        if (Complaint.Model.People.onInitialized) {
            Complaint.Model.People.onInitialized();
        }
        if (Complaint.Model.Documents.onInitialized) {
            Complaint.Model.Documents.onInitialized();
        }
        if (Complaint.Model.Notes.onInitialized) {
            Complaint.Model.Notes.onInitialized();
        }
        if (Complaint.Model.References.onInitialized) {
            Complaint.Model.References.onInitialized();
        }
        if (Complaint.Model.Tasks.onInitialized) {
            Complaint.Model.Tasks.onInitialized();
        }
        if (Complaint.Model.Location.onInitialized) {
            Complaint.Model.Location.onInitialized();
        }
        if (Complaint.Model.History.onInitialized) {
            Complaint.Model.History.onInitialized();
        }
        if (Complaint.Model.Time.onInitialized) {
            Complaint.Model.Time.onInitialized();
        }
        if (Complaint.Model.Cost.onInitialized) {
            Complaint.Model.Cost.onInitialized();
        }
    }
    ...
    , Tree: {
        create: function () {
            if (Complaint.Model.Tree.Key.create) {
                Complaint.Model.Tree.Key.create();
            }
        }
        , onInitialized: function () {
            if (Complaint.Model.Tree.Key.onInitialized) {
                Complaint.Model.Tree.Key.onInitialized();
            }
        }
...
//             }
    ...
    , Lookup: {
        ...
        , refreshAssigneesAndGroups: function () {
            ...
            var complaint = Complaint.Model.getComplaint();
            if (Acm.isNotArray(complaintId) && Complaint.Model.Detail.validateComplaint(complaint))
            {

                var assignees = Complaint.Model.Lookup.getAssignees(complaintId);
                if (Acm.isEmpty(assignees)) {
                    Complaint.Service.Lookup.retrieveAssignees(complaint);
                } else {
                    Complaint.Controller.modelFoundAssignees(assignees);
                }
