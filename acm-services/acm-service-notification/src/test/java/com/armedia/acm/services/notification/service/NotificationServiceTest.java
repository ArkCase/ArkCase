...
public class NotificationServiceTest extends EasyMockSupport
{
    ...
    /*
     * @Test public void testRunEmailSentStartTLS() throws AcmEncryptionException { List<Notification> notifications =
     * new ArrayList<>();
     * 
     * Notification notification1 = new Notification(); notification1.setUser("user"); notification1.setTitle("title");
     * notification1.setNote("note"); notification1.setType("type"); notification1.setParentId(2L);
     * notification1.setParentType("parent type"); notification1.setParentName("parent name");
     * notification1.setParentTitle("parent title"); notification1.setUserEmail("user email");
     * notification1.setStatus("status"); notification1.setAction("action"); notification1.setData("data");
     * notification1.setState("state");
     * 
     * Notification notification2 = new Notification(); notification2.setUser("user"); notification2.setTitle("title");
     * notification2.setNote("note"); notification2.setType("type"); notification2.setParentId(2L);
     * notification2.setParentType("parent type"); notification2.setParentName("parent name");
     * notification2.setParentTitle("parent title"); notification2.setUserEmail("user email");
     * notification2.setStatus("status"); notification2.setAction("action"); notification2.setData("data");
     * notification2.setState("state");
     * 
     * notifications.add(notification1); notifications.add(notification2);
     * 
     * Map<String, Object> messageProps = new HashMap<>(); messageProps.put("host", "host"); messageProps.put("port",
     * "port"); messageProps.put("user", "user"); messageProps.put("password", "password"); messageProps.put("from",
     * "from"); messageProps.put("to", "user email"); messageProps.put("subject", "title");
     * 
     * String lastRunDate = "1970-01-01T00:00:00Z";
     * 
     * BasicNotificationRule assignRule = new BasicNotificationRule(); assignRule.setGlobalRule(true);
     * assignRule.setJpaQuery("query"); assignRule.setQueryType(QueryType.CREATE); assignRule.setExecutor(sendExecutor);
     * 
     * BasicNotificationRule unassignRule = new BasicNotificationRule(); unassignRule.setGlobalRule(true);
     * unassignRule.setJpaQuery("query"); unassignRule.setQueryType(QueryType.CREATE);
     * unassignRule.setExecutor(sendExecutor);
     * 
     * Map<String, NotificationRule> rules = new HashMap<>(); rules.put("assignRule", assignRule);
     * rules.put("unassignRule", unassignRule);
     * 
     * NotificationSenderFactory notificationSenderFactory = new NotificationSenderFactory();
     * notificationSenderFactory.setPropertyFileManager(mockPropertyFileManager); Map<String, NotificationSender>
     ...
     * smtpStartTLSNotificationSender.setPropertyFileManager(mockPropertyFileManager);
     * 
     * notificationSenderMap.put("smtp", smtpNotificationServer); notificationSenderMap.put("smtp+starttls",
     * smtpStartTLSNotificationSender); notificationSenderFactory.setNotificationSenderMap(notificationSenderMap);
     * 
     * NotificationUtils mockNotificationUtils = createMock(NotificationUtils.class);
     * smtpNotificationServer.setNotificationUtils(mockNotificationUtils);
     * smtpStartTLSNotificationSender.setNotificationUtils(mockNotificationUtils);
     * 
     * Map<String, NotificationSenderFactory> senders = new HashMap<>(); senders.put("notificationSender",
     * notificationSenderFactory);
     * 
     * // I am using the same captures below multiple times because we don't need to check these captures
     * Capture<String> stringCapture = new Capture<>(); Capture<Map<String, String>> mapCapture = new Capture<>();
     * Capture<Map<String, Object>> propertiesCapture = new Capture<>(); Capture<NotificationRule> ruleCapture =
     * Capture.newInstance();
     * 
     * expect(mockPropertyFileManager.load(capture(stringCapture), eq("notification.user.last.run.date"),
     * capture(stringCapture))) .andReturn(lastRunDate).anyTimes();
     * mockPropertyFileManager.storeMultiple(capture(mapCapture), capture(stringCapture), eq(true));
     * expectLastCall().anyTimes();
     * expect(mockSpringContextHolder.getAllBeansOfType(NotificationRule.class)).andReturn(rules).anyTimes();
     * expect(mockSpringContextHolder.getAllBeansOfType(NotificationSenderFactory.class)).andReturn(senders).anyTimes();
     * expect(mockNotificationDao.executeQuery(capture(propertiesCapture), eq(0), eq(10),
     * capture(ruleCapture))).andReturn(notifications) .anyTimes();
     * mockAuditPropertyEntityAdapter.setUserId(eq("NOTIFICATION-BATCH-INSERT")); expectLastCall().anyTimes();
     * expect(mockPropertyFileManager.load(capture(stringCapture), eq("notification.user.email.host"),
     * capture(stringCapture))) .andReturn("host").anyTimes();
     * expect(mockPropertyFileManager.load(capture(stringCapture), eq("notification.user.email.port"),
     * capture(stringCapture))) .andReturn("port").anyTimes();
     * expect(mockPropertyFileManager.load(capture(stringCapture), eq("notification.user.email.user"),
     * capture(stringCapture))) .andReturn("user").anyTimes();
     * expect(mockPropertyFileManager.load(capture(stringCapture), eq("notification.user.email.password"),
     * capture(stringCapture))) .andReturn("password").anyTimes();
     * expect(mockPropertyFileManager.load(capture(stringCapture), eq("notification.user.email.from"),
     * capture(stringCapture))) .andReturn("from").anyTimes();
     * expect(mockPropertyFileManager.load(capture(stringCapture), eq("notification.user.email.flow.type"),
     * capture(stringCapture))) .andReturn("smtp").anyTimes();
     * expect(mockPropertyFileManager.load(capture(stringCapture), eq("notification.user.email.flow.starttls"),
     * capture(stringCapture))) .andReturn("true").anyTimes(); try { Capture<Map<String, Object>> messagePropsCapture =
     * EasyMock.newCapture(); expect(mockMuleContextManager.send(eq("vm://sendEmailViaSmtpStartTLS.in"),
     * contains("note"), capture(messagePropsCapture))) .andReturn(mockMuleMessage).anyTimes(); } catch (MuleException
     * e) {
     * 
     * }
     * 
     ...
     * 
     * Capture<ApplicationNotificationEvent> capturedEvent = new Capture<>();
     * mockNotificationEventPublisher.publishNotificationEvent(capture(capturedEvent)); expectLastCall().anyTimes();
     * expect(mockNotificationDao.executeQuery(capture(propertiesCapture), eq(10), eq(10), capture(ruleCapture)))
     * .andReturn(new ArrayList<>()).anyTimes();
     * expect(mockNotificationFormatter.replaceFormatPlaceholders(notification1)).andReturn(notification1).atLeastOnce()
     * ;
     * expect(mockNotificationFormatter.replaceFormatPlaceholders(notification2)).andReturn(notification2).atLeastOnce()
     * ; expect(mockNotificationUtils.buildNotificationLink(anyString(), anyLong(), anyString(),
     * anyLong())).andReturn(null).anyTimes();
     * 
     * replayAll();
     * 
     * notificationService.run();
     ...
     * verifyAll(); }
     * 
     * @Test public void testRunEmailNotSentStartTLS() throws AcmEncryptionException { List<Notification> notifications
     * = new ArrayList<>();
     * 
     * Notification notification1 = new Notification(); notification1.setUser("user"); notification1.setTitle("title");
     * notification1.setNote("note"); notification1.setType("type"); notification1.setParentId(2L);
     ...
     * capture(stringCapture))) .andReturn("smtp").anyTimes();
     * expect(mockPropertyFileManager.load(capture(stringCapture), eq("notification.user.email.flow.starttls"),
     * capture(stringCapture))) .andReturn("true").anyTimes();
     * 
     * expect(mockNotificationFormatter.replaceFormatPlaceholders(notification1)).andReturn(notification1).atLeastOnce()
     * ;
     * expect(mockNotificationFormatter.replaceFormatPlaceholders(notification2)).andReturn(notification2).atLeastOnce()
     ...
     * anyLong())).andReturn(null).anyTimes();
     * 
     * try { Capture<Map<String, Object>> messagePropsCapture = EasyMock.newCapture();
     * expect(mockMuleContextManager.send(eq("vm://sendEmailViaSmtpStartTLS.in"), contains("note"),
     * capture(messagePropsCapture))) .andReturn(mockMuleMessage).anyTimes(); } catch (MuleException e) {
     * 
     * }
     * 
     * // Return Exception - UNSUCCESSFULLY SENT
     * expect(mockMuleMessage.getInboundProperty(eq("sendEmailException"))).andReturn(new
     * Exception("exception")).anyTimes();
     * 
     * Capture<Notification> capturedNotification = new Capture<>();
     * expect(mockNotificationDao.save(capture(capturedNotification))).andReturn(notification1).anyTimes();
     * 
