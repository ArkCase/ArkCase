 ...
 */
public class MediaEngineRemoveProcessIdDelegate implements JavaDelegate
{
    ...
    {
        LOG.debug("Remove Process delegate execution for PROCESS_ID=[{}]", delegateExecution.getProcessInstanceId());

        String serviceName = null;
        try
        {
            serviceName = (String) delegateExecution.getVariable(MediaEngineBusinessProcessVariableKey.SERVICE_NAME.toString());
            getMediaEngineServiceFactory().getService(serviceName).removeProcessId(delegateExecution);
        }
        catch (Exception e)
        {
            LOG.error("Unable to remove processId, service [{}] not found", serviceName, e.getMessage(), e);
        }
    }
