...
public class MediaEngineProviderFailedListener implements ApplicationListener<MediaEngineProviderFailedEvent>
{
    ...
    @Override
    public void onApplicationEvent(MediaEngineProviderFailedEvent event)
    {
        if (event != null && event.isSucceeded())
        {
            ...
            MediaEngine mediaEngine = event.getSource();

            try
            {
                MediaEngineService service = getMediaEngineServiceFactory().getService(serviceName);
                service.fail(mediaEngine.getId(), event.getEventDescription());
            }
            catch (SaveMediaEngineException | MediaEngineServiceNotFoundException e)
            {
                LOG.error("Could not set as failed MediaEngine with ID=[{}], for Service=[{}]. REASON=[{}]", mediaEngine.getId(),
