...
public class MediaEnginePurgeDelegate implements JavaDelegate
{
    ...
    {
        LOG.debug("Purge delegate execution for PROCESS_ID=[{}]", delegateExecution.getProcessInstanceId());

        String serviceName = null;
        try
        {
            serviceName = (String) delegateExecution.getVariable(MediaEngineBusinessProcessVariableKey.SERVICE_NAME.toString());
            getMediaEngineServiceFactory().getService(serviceName).purge(delegateExecution);
        }
        catch (Exception e)
        {
            LOG.error("Unable to execute purge, service [{}] not found", serviceName, e.getMessage(), e);
        }
