...
public class AddParticipantAPIController
{
    ...
    {
        log.info("Participant {} with participant type {} will be added on object [{}]:[{}]", userId, participantType, objectType,
                objectId);

        AcmParticipant returnedParticipant = getAcmParticipantService().getParticipantByParticipantTypeAndObjectTypeAndId(userId,
                participantType, objectType, objectId);
        ...
        else
        {
            ...
            {
                log.error("Exception occurred while trying to add the Participant {} on object [{}]:[{}] as a {}", userId, objectType,
                        objectId, participantType, e);
                 ...
                 * }
                 */
                throw new AcmCreateObjectFailedException(ParticipantConstants.OBJECT_TYPE,
                        "Participant " + userId + " was not added on object['" + objectType + "]:[" + objectId + "] as a " + participantType
                                + " and there is no row inserted into DB due to exception: ",
                        e);
            }
        }
