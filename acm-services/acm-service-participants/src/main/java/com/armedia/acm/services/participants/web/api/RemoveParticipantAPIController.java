...
public class RemoveParticipantAPIController
{
            ...
            Authentication authentication) throws AcmUserActionFailedException
    {
        ...
        {
            log.info("Participant " + userId + " with participant type:" + participantType + " will be removed from object['" + objectType
                    + "]:[" + objectId + "]");
        }
        AcmParticipant participant = getAcmParticipantService().getParticipantByParticipantTypeAndObjectTypeAndId(userId, participantType,
                objectType, objectId);
        try
        {
            if (participant != null
                    && (!participant.getParticipantType().equals("owning group") && !participant.getParticipantType().equals("assignee")))
            {
            ...
            {
                if (log.isDebugEnabled())
                    log.debug("Association not found between Participant" + userId + " and object['" + objectType + "]:[" + objectId
                            + "] in  the DB");
        ...
        catch (Exception e)
        {
            if (log.isErrorEnabled())
                log.error("SQL Exception was thrown while removing participant " + userId + " from object['" + objectType + "]:[" + objectId
                        + "]");
            throw new AcmUserActionFailedException(ParticipantConstants.USER_ACTION_DELETE, ParticipantConstants.OBJECT_TYPE,
                    participant.getId(), "SQL Exception was thrown while deleting Participant", e);
        }
