...
/**
 * Read Alfresco audit records generated by the Alfresco NodeService.  Alfresco uses the NodeService to create folders,
 * and also to create thumbnails and some other objects (but NOT to create content files... Alfresco uses the
 * FileFolderService to create content files).
 * <p>
 * For now, from the node service we are interested only in new folders.  This reader ignores all other new
 * content types.
 */
public class AlfrescoNodeServiceAuditResponseReader implements CreateNodesResponseReader
{
