...
public class FileEventPublisher implements ApplicationEventPublisherAware {
    ...
    public void publishFileCreatedEvent( EcmFile source, Authentication auth, boolean succeeded ) {
        if ( log.isDebugEnabled() ) {
            log.debug("Publishing a file created event.");
        }
        EcmFileCreatedEvent fileCreatedEvent = new EcmFileCreatedEvent(source, auth.getName());
    ...
    public void publishFileDeletedEvent( EcmFile source, Authentication auth, boolean succeeded ) {
        if ( log.isDebugEnabled() ) {
            log.debug("Publishing a file deleted event.");
        }
        EcmFileDeletedEvent fileDeletedEvent = new EcmFileDeletedEvent(source,auth.getName());
        fileDeletedEvent.setSucceeded(succeeded);

        eventPublisher.publishEvent(fileDeletedEvent);
    }

    public void publishFileCopiedEvent( EcmFile source, Authentication auth, boolean succeeded ) {
        if ( log.isDebugEnabled() ) {
            log.debug("Publishing a file copied event.");
        }
        EcmFiledCopiedEvent filedCopiedEvent = new EcmFiledCopiedEvent(source,auth.getName());
        filedCopiedEvent.setSucceeded(succeeded);

        eventPublisher.publishEvent(filedCopiedEvent);
    }

    public void publishFileMovedEvent( EcmFile source, Authentication auth, boolean succeeded ) {
        if ( log.isDebugEnabled() ) {
            log.debug("Publishing a file moved event.");
        }
        EcmFileMovedEvent fileMovedEvent = new EcmFileMovedEvent(source,auth.getName());
        fileMovedEvent.setSucceeded(succeeded);

        eventPublisher.publishEvent(fileMovedEvent);
    }

    public void publishFileRenamedEvent( EcmFile source, Authentication auth, boolean succeeded ) {
        if ( log.isDebugEnabled() ) {
            log.debug("Publishing a file renamed event.");
        }
        EcmFileRenamedEvent fileRenamedEvent = new EcmFileRenamedEvent(source,auth.getName());
        fileRenamedEvent.setSucceeded(succeeded);

        eventPublisher.publishEvent(fileRenamedEvent);
    }

    public void publishFileReplacedEvent( EcmFile source, Authentication auth, boolean succeeded ) {
        if ( log.isDebugEnabled() ) {
            log.debug("Publishing a file replaced event.");
        }
        EcmFileReplacedEvent fileReplacedEvent = new EcmFileReplacedEvent(source,auth.getName());
