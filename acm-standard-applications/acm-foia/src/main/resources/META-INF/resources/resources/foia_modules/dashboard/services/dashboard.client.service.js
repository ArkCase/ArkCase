...
angular.module('dashboard').factory('Dashboard.DashboardService',
        [ '$resource', '$timeout', '$translate', '$q', 'Acm.StoreService', 'UtilService', 'Config.LocaleService', 'ArkCaseDashboard', 'Authentication', function($resource, $timeout, $translate, $q, Store, Util, LocaleService, ArkCaseDashboard, Authentication) {
            var Service = $resource('', {}, {
                ...
                queryNewComplaints: {
                    method: 'GET',
                    url: 'api/v1/plugin/search/advancedSearch?q=object_type_s\\:COMPLAINT+' + 'AND+create_date_tdt\\:%5BNOW/DAY-1MONTH TO NOW%252B1DAY%5D%26rows=100',
                    isArray: false,
                    data: ''
                },
                queryNewCases: {
                    method: 'GET',
                    url: 'api/v1/plugin/search/advancedSearch?q=object_type_s\\:CASE_FILE+' + 'AND+NOT status_lcs\\:DELETED AND create_date_tdt\\:%5BNOW/DAY-1MONTH TO NOW%252B1DAY%5D%26rows=100',
                    isArray: false
                },
                queryMyTasks: {
                    method: 'GET',
                ...
                queryMyOverdueRequests: {
                    method: 'GET',
                    url: 'api/v1/plugin/search/advancedSearch?q=object_type_s\\:CASE_FILE+' + 'AND+dueDate_tdt\\:%5B*+TO+NOW%5D' + '+AND+assignee_id_lcs\\::userId+' + 'AND+' + '-status_lcs\\::status&n=:pageSize&start=:startWith&s=:sortBy :sortDir',
                    isArray: false,
                    data: ''
                }
            });
