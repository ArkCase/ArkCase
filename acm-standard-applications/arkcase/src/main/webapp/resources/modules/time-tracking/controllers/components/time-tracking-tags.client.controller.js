...
angular.module('time-tracking').controller(
        ...
        [ '$scope', '$q', '$stateParams', '$translate', 'UtilService', 'ObjectService', 'TimeTracking.InfoService', 'Helper.UiGridService', 'Helper.ObjectBrowserService', 'Object.TagsService', '$modal', 'MessageService',
                function($scope, $q, $stateParams, $translate, Util, ObjectService, TimeTrackingInfoService, HelperUiGridService, HelperObjectBrowserService, ObjectTagsService, $modal, messageService) {
                                ...
                                if (tag.id) {
                                    if (tag.object_id_s) {
                                        ...
                                        if (tagsFound.length == 0) {
                                            ObjectTagsService.associateTag(componentHelper.currentObjectId, ObjectService.ObjectTypes.TIMESHEET, $scope.objectParentTitle, tag.object_id_s).then(function(returnedTag) {
                                        ...
                                        ObjectTagsService.associateTag(componentHelper.currentObjectId, ObjectService.ObjectTypes.TIMESHEET, $scope.objectParentTitle, tag.id).then(function() {
                                            $scope.tags.push(tag);
                                            $scope.gridOptions.data = $scope.tags;
                                            $scope.gridOptions.totalItems = $scope.tags.length;
                                        });
