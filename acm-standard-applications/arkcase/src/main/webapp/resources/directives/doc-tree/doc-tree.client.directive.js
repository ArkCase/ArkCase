            ...
            function ($q, $translate, $modal, $filter, $log, $injector, Store, Util, UtilDateService, ConfigService, PluginService, UserInfoService, Ecm, EmailSenderConfigurationService, LocaleHelper, LookupService, MessageService, ObjectService, FileUploaderConfigurationService, DeDuplicationConfigurationService, $timeout, MessageHandler, MedicalComprehendService, ZylabIntegrationService) {
                ...
                var DocTree = {
                        ...
                        create: function(jqTree) {
                            ...
                            jqTree.fancytree(treeArgsToUse).on("command", DocTree.Command.onCommand).on("mouseenter", ".fancytree-node", function(event) {
                                        ...
                                        if (isReadOnly) {
                                            ...
                                            if (Util.isArray(node.data.versionList)) {
                                                for (var v = 0; v < node.data.versionList.length; v++) {
                                                    ...
                                                    var $option = $("<option/>").val(versionTag).text(versionTag).appendTo($select);
                        ...
                        Command: {
                            ...
                            executeSubmitFiles: function(nodes, args) {
                                ...
                                var getFileUploadSettings = FileUploaderConfigurationService.getFileUploaderConfiguration();

                                var promiseUploadFile = null;

                                $q.when(getFileUploadSettings).then(function(settings) {
                                    promiseUploadFile = DocTree.doSubmitFormUploadFile(files, fileLang, settings);
                                ...
                                return [ {
                                    ...
                                    execute: function (nodes, args) {
                                        ...
                                        if (!Util.isEmpty(ecmFile) && !Util.isEmpty(ecmFile.version) && !Util.isArrayEmpty(ecmFile.versionList)) {
                                            var activeVersion = _.find(ecmFile.versionList, function (version) {
                                                return ecmFile.version === version.versionTag;
                                            });
                                            ...
                                            MedicalComprehendService.getComprehendMedicalByMediaId(activeVersion.id).then(function (result) {
                                                ...
                                                if (_.isEmpty(comprehendMedicalResult)) {
                                                    MedicalComprehendService.createAutomaticComprehendMedical(activeVersion.id);
                                                } else {
                                                    MessageService.info($translate.instant('common.directive.docTree.nlpProcess.messages.'
                                                        + comprehendMedicalResult.status.toLowerCase()));
                                                }
                                            }, function (error) {
                        ...
                        makeTable: function(element, columnDefs) {
                            var jqTable = $("<table/>").addClass("table table-striped th-sortable table-hover").appendTo($(element));
                        ...
                        makeUploadDocForm: function(jqTree) {
                            this.jqFormUploadDoc = $("<form/>").attr("id", "formUploadDoc").attr("style", "display:none;").appendTo(jqTree);
                            this.jqFileInput = $("<input/>").attr("type", "file").attr("id", "file").attr("name", "files[]").attr("multiple", "").appendTo(this.jqFormUploadDoc);
                        ...
                        makeDownloadDocForm: function(jqTree) {
                            this.jqFormDownloadDoc = $("<form/>").attr("id", "formDownloadDoc").attr("action", "#").attr("style", "display:none;").appendTo(jqTree);
                        }
                        ...
                        ,
                        doSubmitFormUploadFile: function(files, fileLang, settings) {
                            ...
                            if (DocTree.uploadSetting.uploadFileNew) {
                                if (settings.data["fileUploader.enableFileChunkUpload"]) {
                            ...
                            if (node) {
                                ...
                                if (parent) {
                                    ...
                                    if (verSelected != verCurrent) {
                                        if (verSelected < verCurrent) {
                                            Ui.dlgConfirm($translate.instant("common.directive.docTree.confirmVersion"), function(result) {
                                                if (result) {
                                ...
                                controller: [
                                        ...
                                        function($rootScope, $scope, $interval, params) {
                                            ...
                                            function checkFrevvoSubmission() {
                                                if (!Util.isEmpty($rootScope.frevvoMessaging) && !Util.isEmpty($rootScope.frevvoMessaging.receiver) && !Util.isEmpty($rootScope.frevvoMessaging.receiver.document) && !Util.isEmpty($rootScope.frevvoMessaging.receiver.document.body)
                                                        && !Util.isEmpty($rootScope.frevvoMessaging.receiver.document.body.getElementsByTagName('div')) && $rootScope.frevvoMessaging.receiver.document.body.getElementsByTagName('div').length > 0
                                                        && !Util.isEmpty($rootScope.frevvoMessaging.receiver.document.body.getElementsByTagName('div')[0]) && !Util.isEmpty($rootScope.frevvoMessaging.receiver.document.body.getElementsByTagName('div')[0].innerHTML.trim())
                    ...
                    return {
                        ...
                        link: function(scope, element, attrs) {
                            ...
                            PluginService.getConfigurablePlugins().$promise.then(function(data) {
                            ...
                            scope.$watchGroup([ 'treeConfig', 'objectInfo', 'treeConfig.fileTypes', 'treeConfig.fileLanguages' ], function(newValues, oldValues, scope) {
                                ...
                                promiseCommon.then(function() {
                                    ...
                                    if (Util.goodMapValue(DocTree.treeConfig, "columnDefs")) {
                                        ...
                                        DocTree.fileTypes = Util.goodMapValue(DocTree.treeConfig, "fileTypes", []).concat(Util.goodMapValue(DocTree.treeConfig, "formTypes", []));
                                        var jqTreeBody = DocTree.jqTree.find("tbody");
                                        DocTree.Menu.useContextMenu(jqTreeBody);
...
angular.module('directives').controller('directives.DocTreeDndDialogController', [ '$scope', '$modalInstance', 'UtilService', 'OpTypes', 'params', function($scope, $modalInstance, Util, OpTypes, params) {
