...
public class CreatePrivacyAlfrescoAndRmFoldersExecutor implements AcmDataUpdateExecutor
{

    public static final String SAR_FOLDER_NAME = "SAR";
    ...
    {
        return "create-privacy-alfresco-and-rm-folders";
    }
    ...
    public void execute()
    {
        try
        {
            log.info("Creating Privacy Alfresco folders");

            String defaultBasePath = getEcmFileConfig().getDefaultBasePath();

            String sarFolderName = defaultBasePath + "/" + SAR_FOLDER_NAME;
            getEcmFileService().createFolder(sarFolderName, ArkCaseCMISConstants.DEFAULT_CMIS_REPOSITORY_ID);

            log.info("Creating Privacy Alfresco folders successfully");

            log.info("Creating Privacy record categories");

            getAlfrescoRecordsService().createNewRecordCategoryInBaseFolder(SAR_FOLDER_NAME);

            log.info("Creating Privacy record categories successfully");
        }
        catch (AcmCreateObjectFailedException | AlfrescoServiceException e)
        {
            log.error("Error on creating Privacy Alfresco structure and RM structure");
        }
