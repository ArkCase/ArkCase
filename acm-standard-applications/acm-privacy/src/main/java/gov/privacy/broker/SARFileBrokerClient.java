...
import gov.privacy.service.ResponseFolderCompressorService;

/**
 * Sends SAR files to outbound queue
 *
 * @author Vladimir Cherepnalkovski <vladimir.cherepnalkovski@armedia.com> on Jun, 2020
 */
public class SARFileBrokerClient extends AcmFileBrokerClient
{
    ...
    public SARFileBrokerClient(ActiveMQConnectionFactory connectionFactory, String outboundQueue, String inboundQueue)
    {
    ...
    private void sendReleaseFile(CaseFile request, String filePath)
    {
        ...
        if (!file.exists())
        {
            LOG.error("Required SAR release file was not found on file system " + file);
            return;
        }
        ...
        try
        {
            LOG.debug("Sending SAR release file " + file + " with properties " + properties);
            sendFile(file, properties);
        }
        catch (JMSException | IOException e)
        {
            LOG.error("Failed to send SAR release file", e);
        }
    }
