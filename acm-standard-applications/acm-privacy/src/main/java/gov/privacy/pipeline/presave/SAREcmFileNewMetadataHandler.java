 ...
 *
 */
public class SAREcmFileNewMetadataHandler extends EcmFileNewMetadataHandler
{
    ...
    public void execute(EcmFile entity, EcmFileTransactionPipelineContext pipelineContext) throws PipelineProcessException
    {
        ...
        if (!pipelineContext.getIsAppend())
        {
            ...
            SARFile fileMetadata = new SARFile();
            try
            {
            ...
            {
                log.error("Could not copy properties from EcmFile to SARFile");
            }
            SARFile saved = (SARFile) getEcmFileDao().save(fileMetadata);
