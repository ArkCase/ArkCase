                ...
                function($q, $translate, Util, SearchService, SearchQueryBuilder, $injector) {
                    return {
                        ...
                        link : function(scope, el, attrs) {
                            ...
                            scope.onNoDataMessage = Util.goodValue(scope.onNoDataMessage, $translate
                                    .instant("common.directive.searchModal.noData.text"));
                            ...
                            scope.pickUserLabel = Util.goodValue(scope.pickUserLabel, $translate
                                    .instant("common.directive.searchModal.pickUserLabel"));
                            scope.pickGroupLabel = Util.goodValue(scope.pickGroupLabel, $translate
                                    .instant("common.directive.searchModal.pickGroupLabel"));
                            scope.cancel = Util.goodValue(scope.cancel, $translate.instant("common.directive.searchModal.btnCancel.text"));
                            scope.searchPlaceholder = Util.goodValue(scope.searchPlaceholder, $translate
                                    .instant("common.directive.searchModal.edtPlaceholder"));
                            ...
                            scope.queryExistingItems = function() {
                                if (!Util.isEmpty(scope.searchQuery)) {
                                    ...
                                    if (scope.findGroups) {
                                        query = SearchQueryBuilder.buildSafeFqFacetedSearchQuerySorted('*', scope.filters, scope.pageSize,
                                                scope.start, scope.sort);
                                    } else {
                                        query = SearchQueryBuilder.buildSafeFqFacetedSearchQuerySorted(scope.searchQuery + '*',
                                                scope.filters, scope.pageSize, scope.start, scope.sort);
                                    }

                                    if (query) {
                                        scope.showNoData = false;
                                        if (!Util.isEmpty(scope.externalSearchServiceName)
                                                && !Util.isEmpty(scope.externalSearchServiceParams)
                                                && !Util.isEmpty(scope.externalSearchServiceParams.organizationId)) {
                                            ...
                                            scope.externalSearchService[scope.externalSearchServiceMethod](
                                                    scope.externalSearchServiceParams, successSearchResult);
                                        } else {
                            ...
                            scope.querySubItems = function(filters) {
                                if (!Util.isEmpty(filters)) {
                                    var query = SearchQueryBuilder.buildSafeFqFacetedSearchQuerySorted('*', filters, scope.pageSizeSecond,
                                            scope.startSecond, scope.sortSecond);
                            ...
                            if (scope.defaultFilter) {
                                var query = SearchQueryBuilder.buildSafeFqFacetedSearchQuerySorted(scope.searchQuery + '*',
                                        scope.defaultFilter, scope.pageSize, 0, scope.sort);
                                if (query) {
                                    scope.showNoData = true;
                                    SearchService.queryFilteredSearch({
                                        query : query
