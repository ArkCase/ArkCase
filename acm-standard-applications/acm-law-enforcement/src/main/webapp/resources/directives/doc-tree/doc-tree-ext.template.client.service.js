 ...
 * @description
 *
 * {@link /acm-standard-applications/acm-law-enforcement/src/main/webapp/resources/directives/doc-tree/doc-tree-ext.template.client.service.js directives/doc-tree/doc-tree-ext.template.client.service.js}
 *
 * DocTree extensions for Correspondence Template functions.
 */
angular.module('services').factory('DocTreeExt.Template', [ '$q', 'UtilService', 'Object.CorrespondenceService', function($q, Util, ObjectCorrespondenceService) {
        ...
        getCommandHandlers: function(DocTree) {
            ...
            return [ {
                ...
                getArgs: function(data) {
                    return {
                        templateType: data.cmd.substring(this.name.length),
                        label: data.label
                    };
                },
                execute: function(nodes, args) {
                    ...
                    if (!Util.isArrayEmpty(names)) {
                        if(names[0] === "Multi Correspondence") {
                            var data = {
                                nodes: nodes,
                                args: args,
                                docTree: DocTree
                            };
                            DocTree.scope.$bus.publish('multi-correspondence-requested', data);
                        }
                        else {
                            DocTree._addingFileNodes(nodes[0], names, names[0]).then(function(data) {
                                var selectedFolderId = nodes[0].data.objectId;
                                ObjectCorrespondenceService.createCorrespondence(template, DocTree.getObjType(), DocTree.getObjId(), selectedFolderId).then(function(uploadedFile) {
