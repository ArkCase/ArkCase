...
angular.module('profile').controller('Profile.MfaFactorModalController', ['$scope', '$modalInstance', 'UtilService'
    , 'Profile.MfaService', 'factorInfo', 'isEdit', 'availableFactors'
    , function ($scope, $modalInstance, Util, ProfileMfaService, factorInfo, isEdit, availableFactors) {
        ...
        $scope.disableConfirm = function () {
            ...
            if (Util.compare('email', $scope.factorInfo.factorType)) {
                return Util.isEmpty($scope.factorInfo.profile) ||
                    Util.isEmpty($scope.factorInfo.profile.email);
            } else if (Util.compare('sms', $scope.factorInfo.factorType)) {
                return Util.isEmpty($scope.factorInfo.profile) ||
                    Util.isEmpty($scope.factorInfo.profile.phoneNumber) ||
                    $scope.factorInfo.profile.phoneNumber.length < 7;
