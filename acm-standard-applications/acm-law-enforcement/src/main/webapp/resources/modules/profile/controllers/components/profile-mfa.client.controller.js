...
angular.module('profile').controller('Profile.MfaController', ['$scope', '$modal', '$translate', '$state', 'ConfigService'
    , 'UtilService', 'MessageService', 'Profile.MfaService', 'i18nConstants'
    , function ($scope, $modal, $translate, $state, ConfigService, Util, MessageService, ProfileMfaService, i18nConstants) {
        /**
        ...
        $scope.isFactorEditDisabled = function (rowEntity) {
            return Util.isEmpty(rowEntity) ||
                Util.isEmpty(rowEntity.factorType) ||
                Util.isEmpty(rowEntity.id) ||
                Util.compare('PENDING_ACTIVATION', rowEntity.status) ||
                Util.compare('token:software:totp', rowEntity.factorType);
        };
