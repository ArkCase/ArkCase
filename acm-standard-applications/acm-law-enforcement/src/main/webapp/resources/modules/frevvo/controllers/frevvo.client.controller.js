...
angular
        ...
        .controller(
                ...
                [
                        ...
                        function($rootScope, $scope, $stateParams, $sce, $q, $modal, $translate, $interval, Util, ConfigService,
                                TicketService, LookupService, FrevvoFormService, ServCommService, PersonInfoService, ObjectLookupService,
                                ...
                                if (!Util.isEmpty(frevvoIframe)) {
                                    ...
                                    if (Util.isEmpty($rootScope.frevvoMessaging)) {
                                        ...
                                        $rootScope.frevvoMessaging.receive = function receive(e) {
                                            if (!Util.isEmpty(e) && !Util.isEmpty(e.data) && !Util.isEmpty(e.data.source)
                                                    && e.data.source == "frevvo" && !Util.isEmpty(e.data.action)) {
                            ...
                            function pickObject(data) {
                                ...
                                if (!Util.isEmpty(data.data) && !Util.isEmpty(data.data.objectType) && data.data.objectType == 'OTHER') {
                                    var customParams = {};
                                    customParams.chargeCodes = Util.goodMapValue($scope.config,
                                            "timesheetCustomPicker.otherTypeChargeCodes");
                                    ...
                                    modalInstance = $modal.open({
                                        ...
                                        controller : [ '$scope', '$modalInstance', 'customParams',
                                                function($scope, $modalInstance, customParams) {
                                                    $scope.modalInstance = $modalInstance;
                                    ...
                                    modalInstance = $modal.open({
                                        ...
                                        controller : [ '$scope', '$modalInstance', 'ConfigService', 'params',
                                                function($scope, $modalInstance, ConfigService, params) {
                                                    $scope.modalInstance = $modalInstance;
                                                    $scope.header = params.header;
                                                    $scope.filter = params.filter;
                                                    $scope.config = params.config;
                            ...
                            function saveNewPersonAndUpdateFrevvo(data, message) {
                                ...
                                PersonInfoService
                                        .savePersonInfoWithPictures(data.person, data.personImages)
                                        .then(
                                                function(response) {
                                                    returnMessage.personId = response.data.id;
                                                    returnMessage.fullName = (response.data.givenName + " " + response.data.familyName)
                                                            .trim();
