...
angular.module('tasks')
        .controller(
                ...
                [
                        ...
                        'MessageService',
                        function($scope, $state, $stateParams, $modal, $translate, Util, ConfigService, Authentication,
                                ObjectSignatureService, ObjectService, TaskInfoService, TaskWorkflowService, ObjectSubscriptionService,
                            ...
                            var onObjectInfoRetrieved = function(objectInfo) {
                                ...
                                promiseQueryUser.then(function(userInfo) {
                                    ...
                                    ObjectSubscriptionService.getSubscriptions(userInfo.userId, ObjectService.ObjectTypes.TASK,
                                            $scope.objectInfo.taskId).then(function(subscriptions) {
                            ...
                            $scope.sign = function() {
                                ...
                                modalInstance.result.then(function(result) {
                                    if (result) {
                                        ...
                                        ObjectSignatureService.confirmSignature(ObjectService.ObjectTypes.TASK, $scope.objectInfo.taskId,
                                                result.pass).then(function(result) {
                            ...
                            $scope.subscribe = function() {
                                ObjectSubscriptionService
                                        .subscribe($scope.userId, ObjectService.ObjectTypes.TASK, $scope.objectInfo.taskId).then(
                                                function(data) {
                            ...
                            $scope.unsubscribe = function() {
                                ObjectSubscriptionService.unsubscribe($scope.userId, ObjectService.ObjectTypes.TASK,
                                        $scope.objectInfo.taskId).then(function(data) {
                            ...
                            $scope.onClickOutcome = function(name) {
                                ...
                                if (!Util.isEmpty(taskInfo.buckslipFutureApprovers) && taskInfo.buckslipFutureApprovers.length > 0) {
                                    for (var i = 0; i < taskInfo.buckslipFutureApprovers.length; i++) {
                                        taskInfo.buckslipFutureApprovers[i].created = taskInfo.buckslipFutureApprovers[i].created
                                                .split('.')[0]
                                                + "Z";
                                        taskInfo.buckslipFutureApprovers[i].modified = taskInfo.buckslipFutureApprovers[i].modified
                                                .split('.')[0]
                                                + "Z";

                                        if (!Util.isEmpty(taskInfo.buckslipFutureApprovers[i].deleted)) {
                                            taskInfo.buckslipFutureApprovers[i].deleted = taskInfo.buckslipFutureApprovers[i].deleted
                                                    .split('.')[0]
                                                    + "Z";
                                        }
                                    }
                            ...
                            $scope.showClaimBtn = function() {
                                var isClosedStatus = Util.compare(Util.goodMapValue($scope.objectInfo, 'status'), "CLOSED")
                                        || Util.compare(Util.goodMapValue($scope.objectInfo, 'status'), "TERMINATED");
                                if ($scope.objectInfo && !Util.goodMapValue($scope.objectInfo, "assignee", false)) {
                                    if (Util.goodMapValue($scope.objectInfo, "candidateGroups", false)
                                            && !Util.isArrayEmpty($scope.objectInfo.candidateGroups)) {
                                        if (Util.goodMapValue($scope.userInfo, "authorities", false)
                                                && !Util.isArrayEmpty($scope.userInfo.authorities)) {
                                            if (_.includes($scope.userInfo.authorities, "ROLE_ADMINISTRATOR")) {
                                                //Admin users can claim any group task.
                                                return !isClosedStatus;
                                            }
                                            if (!Util.isArrayEmpty(_.intersection($scope.objectInfo.candidateGroups,
                                                    $scope.userInfo.authorities))) {
                                                return !isClosedStatus;
                                            }
                                        }
                                    }
                                ...
                                if ($scope.objectInfo && Util.goodMapValue($scope.objectInfo, "assignee", false)) {
                                    if (Util.goodMapValue($scope.userInfo, "authorities", false)
                                        ...
                                        if (_.includes($scope.userInfo.authorities, "ROLE_ADMINISTRATOR")) {
                                            ...
                                            return true;
                                        } else if (!Util.isArrayEmpty(_.intersection($scope.objectInfo.candidateGroups,
                                                $scope.userInfo.authorities))) {
                                            return true;
                                        }
                                    }
                            ...
                            var saveTask = function(taskInfoUpdated) {
                                ...
                                if (TaskInfoService.validateTaskInfo($scope.objectInfo)) {
                                    var objectInfo = taskInfoUpdated == null ? Util.omitNg($scope.objectInfo) : Util
                                            .omitNg(taskInfoUpdated);
...
angular.module('tasks').controller('Tasks.ActionsErrorDialogController',
        [ '$scope', '$modalInstance', 'errorMessage', function($scope, $modalInstance, errorMessage) {
            $scope.errorMessage = errorMessage;
            $scope.onClickOk = function() {
                $modalInstance.dismiss('cancel');
            };
        } ]);
angular.module('tasks').controller('Tasks.RejectDialogController',
        [ '$scope', '$modalInstance', 'aValue', function($scope, $modalInstance, aValue) {
