...
angular.module('tasks')
        .controller(
                ...
                [
                        ...
                        'modalParams',
                        'moment',
                        function($scope, $state, $sce, $q, $modal, ConfigService, Util, TicketService, LookupService, FrevvoFormService,
                                TaskNewTaskService, Authentication, UtilDateService, DialogService, ObjectService, ObjectLookupService,
                            ...
                            ConfigService.getModuleConfig("tasks").then(
                                    function(moduleConfig) {
                                        ...
                                        if (!Util.isEmpty($scope.modalParams.parentObject) && !Util.isEmpty($scope.modalParams.parentType)
                                                && !Util.isEmpty($scope.modalParams.parentId)) {
                            ...
                            $scope.startDateChanged = function() {
                                ...
                                if (Util.isEmpty($scope.config.data.taskStartDate)
                                        || moment($scope.config.data.taskStartDate).isBefore(todayDate)) {
                                    $scope.config.data.taskStartDate = todayDate;
                                } else {
                                    $scope.config.data.taskStartDate = UtilDateService
                                            .convertToCurrentTime($scope.config.data.taskStartDate);
                                }
                                ...
                                $scope.config.data.dueDate = UtilDateService.setSameTime($scope.config.data.dueDate,
                                        $scope.config.data.taskStartDate);
                            ...
                            $scope.dueDateChanged = function() {
                                ...
                                $scope.config.data.taskStartDate = UtilDateService.setSameTime($scope.config.data.taskStartDate,
                                        $scope.config.data.dueDate);
                            };
                            ...
                            $scope.saveNewTask = function() {
                                ...
                                if ($scope.taskType === 'REVIEW_DOCUMENT' && $scope.documentsToReview) {
                                    ...
                                    TaskNewTaskService.reviewDocuments(taskData, $scope.selectedBusinessProcessType).then(
                                            reviewDocumentTaskSuccessCallback, errorCallback);
                                } else {
