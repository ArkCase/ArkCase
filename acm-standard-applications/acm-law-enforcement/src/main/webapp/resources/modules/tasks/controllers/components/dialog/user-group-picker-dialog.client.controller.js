...
angular
        .module('tasks')
        .controller(
                'Tasks.UserGroupPickerDialogController',
                [
                        '$scope',
                        '$modalInstance',
                        '$q',
                        ...
                        function($scope, $modalInstance, $q, Util, AdminFunctionalAccessControlService, organizationalHierarchyService,
                            ...
                            if ($scope.cfg) {
                                $scope.groupTypeList = Util.goodMapValue($scope.cfg, 'topLevelGroupTypes')
                                        && Util.isArray($scope.cfg.topLevelGroupTypes) && !_.isEmpty($scope.cfg.topLevelGroupTypes) ? $scope.cfg.topLevelGroupTypes
                                        : [ 'LDAP_GROUP' ];
                                $scope.cfg.roleFilters = Util.goodMapValue($scope.cfg, 'roleFilters')
                                        && Util.isArray($scope.cfg.roleFilters) ? $scope.cfg.roleFilters : [];
                                $scope.cfg.treeFilters = _.sortBy(Util.goodMapValue($scope.cfg, 'treeFilters')
                                        && Util.isArray($scope.cfg.treeFilters) ? $scope.cfg.treeFilters : [], 'name');
                                $scope.cfg.allowedFilterRoles = Util.goodMapValue($scope.cfg, 'allowedFilterRoles')
                                        && Util.isArray($scope.cfg.allowedFilterRoles) ? $scope.cfg.allowedFilterRoles : [];
                            ...
                            var buildTreeFilters = function(roleToGroups) {
                                return _.chain(roleToGroups).keys().filter(
                                        function(key) {
                                            return _.isEmpty($scope.cfg.allowedFilterRoles) ? true : !_.isEmpty(_.intersection(
                                                    $scope.cfg.allowedFilterRoles, [ key ]));
                                        }).map(function(key) {
                                    return {
                            ...
                            $scope.onLoadMore = function(currentPage, pageSize) {
                                ...
                                groupsPromise = organizationalHierarchyService.getFilteredTopLevelGroups(currentPage, pageSize,
                                        $scope.groupTypeList, $scope.cfg.roleFilters);
