...
angular
        ...
        .controller(
                ...
                [
                        ...
                        'PermissionsService',
                        'MessageService',
                        function($rootScope, $scope, $q, $stateParams, $translate, $modal, Util, ObjectService, OrganizationInfoService,
                                Authentication, HelperUiGridService, HelperObjectBrowserService, ObjectLookupService,
                                OrganizationSearchService, ObjectAssociationService, $timeout, PermissionsService, MessageService) {
                            ...
                            function refreshGridData(objectId, objectType) {
                                ObjectAssociationService.getObjectAssociations(objectId, objectType, 'ORGANIZATION').then(
                                        function(response) {
                            ...
                            function updateAssociation(association, parent, target, associationData, rowEntity) {
                                ...
                                ObjectAssociationService
                                        .saveObjectAssociation(association)
                                        .then(
                                                function(payload) {
                                                    //success
                                                    if (payload.associationType.toLowerCase() !== "parentcompany") {
                                                        ...
                                                        if (!Util.isEmpty(target.defaultIdentification)) {
                                                            if (!Util.isEmpty(target.defaultIdentification.identificationType)) {
                                                                rowEntity.target_object.default_identification_s = target.defaultIdentification.identificationNumber
                                                                        + " " + target.defaultIdentification.identificationType;
                                                            } else {
                                                                rowEntity.target_object.default_identification_s = target.defaultIdentification.identificationNumber;
                                                            }
                                                        }
                                                        ...
                                                        if (!Util.isEmpty(target.primaryContact)) {
                                                            if (!Util.isEmpty(target.primaryContact.person.familyName)) {
                                                                rowEntity.target_object.primary_contact_s = target.primaryContact.person.givenName
                                                                        + " " + target.primaryContact.person.familyName;
                                                            } else {
                                                                rowEntity.target_object.primary_contact_s = target.primaryContact.person.givenName;
                                                            }
                                                        ...
                                                        if (!Util.isEmpty(target.defaultPhone)) {
                                                            rowEntity.target_object.default_phone_s = target.defaultPhone.value + " ["
                                                                    + target.defaultPhone.subType + "]";
                                                        } else {
