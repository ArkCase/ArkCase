...
angular.module('cost-tracking')
        .controller(
                ...
                [
                        ...
                        '$modal',
                        'MessageService',
                        function($scope, $q, $stateParams, $translate, Util, ObjectService, CostTrackingInfoService, HelperUiGridService,
                                            ...
                                            if (tag.object_id_s) {
                                                ...
                                                if (tagsFound.length == 0) {
                                                    ObjectTagsService.associateTag(componentHelper.currentObjectId,
                                                            ObjectService.ObjectTypes.COSTSHEET, $scope.objectParentTitle, tag.object_id_s)
                                                    ...
                                                    messageService.info(tag.tags_s + " "
                                                            + $translate.instant('costTracking.comp.tags.message.tagAssociated'));
                                                ...
                                                ObjectTagsService.associateTag(componentHelper.currentObjectId,
                                                        ObjectService.ObjectTypes.COSTSHEET, $scope.objectParentTitle, tag.id).then(
                                                        function() {
                                                            $scope.tags.push(tag);
                                                            $scope.gridOptions.data = $scope.tags;
                                                            $scope.gridOptions.totalItems = $scope.tags.length;
                            ...
                            var onObjectInfoRetrieved = function(objectInfo) {
                                ...
                                if (Util.goodPositive(currentObjectId, false)) {
                                    var promiseQueryTags = ObjectTagsService.getAssociateTags(currentObjectId,
                                            ObjectService.ObjectTypes.COSTSHEET);
                            ...
                            $scope.deleteRow = function(rowEntity) {
                                ...
                                if (0 < id) { //do not need to call service when deleting a new row with id==0
                                    ObjectTagsService.removeAssociateTag(componentHelper.currentObjectId,
                                            ObjectService.ObjectTypes.COSTSHEET, rowEntity.id).then(function() {
