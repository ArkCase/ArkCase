...
angular
        ...
        .controller(
                ...
                [
                        ...
                        function($scope, $stateParams, $q, $translate, $filter, ObjectService, ObjectTaskService, Util, CaseInfoService,
                                ComplaintInfoService, TaskInfoService, ConfigService, HelperObjectBrowserService) {
                            ...
                            if (module && Util.goodPositive(currentObjectId, false)) {
                                ObjectTaskService
                                        .queryChildTasks(module.name, currentObjectId, 0, 100, '', '')
                                        .then(
                                                function(queryChildResult) {
                                                    ...
                                                    if (!Util.isEmpty(objectId)) {
                                                        ...
                                                        TaskInfoService
                                                                .getTaskInfo(objectId)
                                                                .then(
                                                                        function(taskInfo) {
                                                                            $scope.objectInfo = taskInfo;
                                                                            if ($scope.objectInfo.buckslipFutureApprovers
                                                                                    && $scope.objectInfo.buckslipPastApprovers) {
                                                                                var data = [];
                                                                                var currentApprover = {};
                                                                                currentApprover.status = "Current";
                                                                                currentApprover.name = $scope.objectInfo.assignee;
                                                                                var dueDateFormated = moment($scope.objectInfo.dueDate)
                                                                                        .format(
                                                                                                $translate
                                                                                                        .instant('common.defaultDateFormat'));
                                                                                currentApprover.date = "Due: " + dueDateFormated;

                                                                                data.push(currentApprover);

                                                                                if (!Util
                                                                                        .isArrayEmpty($scope.objectInfo.buckslipFutureApprovers))
                                                                                ...
                                                                                if (!Util
                                                                                        .isArrayEmpty($scope.objectInfo.buckslipPastApprovers))
                                                                                    for (var i = 0; i < $scope.objectInfo.buckslipPastApprovers.length; i++) {
                                                                                        ...
                                                                                        var doneDateFormated = moment(
                                                                                                $scope.objectInfo.buckslipPastApprovers[i].date)
                                                                                                .format(
                                                                                                        $translate
                                                                                                                .instant('common.defaultDateFormat'));
