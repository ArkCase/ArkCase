 ...
 */
angular.module('services').factory('Case.InfoService', ['$resource', '$translate', 'Acm.StoreService', 'UtilService', 'Object.InfoService', 'CacheFactory',
    function($resource, $translate, Store, Util, ObjectInfoService, CacheFactory) {

        var caseCache = CacheFactory.get('objectCache');
        var caseBaseUrl = 'api/latest/plugin/casefile/byId/';
        ...
        Service.resetCaseInfo = function(caseInfo) {
            if (caseInfo && caseInfo.id) {
                caseCache.remove(caseBaseUrl + caseInfo.id);
            } else {
                caseCache.removeAll();
            }
        };
            ...
            return Util.serviceCall({
                ...
                onSuccess: function(data) {
                    if (Service.validateCaseInfo(data)) {
                        var caseInfo = data;
                        if (caseInfo.id) {
                            caseCache.put(caseBaseUrl + caseInfo.id, caseInfo);
                        }
                        return caseInfo;
                    }
                }
