...
angular.module('cases')
        .controller(
                ...
                [
                        ...
                        '$modal',
                        'MessageService',
                        function($scope, $q, $stateParams, $translate, Util, ObjectService, CaseInfoService, HelperUiGridService,
                                            ...
                                            if (tag.object_id_s) {
                                                ...
                                                if (tagsFound.length == 0) {
                                                    ObjectTagsService.associateTag(componentHelper.currentObjectId,
                                                            ObjectService.ObjectTypes.CASE_FILE, $scope.objectParentTitle, tag.object_id_s)
                                                    ...
                                                    messageService.info(tag.tags_s + " "
                                                            + $translate.instant('cases.comp.tags.message.tagAssociated'));
                                                ...
                                                ObjectTagsService.associateTag(componentHelper.currentObjectId,
                                                        ObjectService.ObjectTypes.CASE_FILE, $scope.objectParentTitle, tag.id).then(
                                                        function() {
                                                            $scope.tags.push(tag);
                                                            $scope.gridOptions.data = $scope.tags;
                                                            $scope.gridOptions.totalItems = $scope.tags.length;
                            ...
                            $scope.deleteRow = function(rowEntity) {
                                ...
                                if (0 < id) { //do not need to call service when deleting a new row with id==0
                                    ObjectTagsService.removeAssociateTag(componentHelper.currentObjectId,
                                            ObjectService.ObjectTypes.CASE_FILE, rowEntity.id).then(function() {
