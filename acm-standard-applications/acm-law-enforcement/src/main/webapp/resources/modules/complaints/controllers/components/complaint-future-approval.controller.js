        ...
        .controller(
                ...
                [
                        ...
                        function($scope, $stateParams, $q, $translate, $modal, Util, UtilDateService, ConfigService, ObjectService,
                                LookupService, ObjectLookupService, ComplaintInfoService, HelperUiGridService, HelperObjectBrowserService,
                                Authentication, PermissionsService, UserInfoService, ObjectTaskService, TaskInfoService,
                                ComplaintFutureApprovalService, MessageService, Store, ModalDialogService, $timeout) {
                            ...
                            var onConfigRetrieved = function(config) {
                                ...
                                gridHelper.addCustomButton(config, "down", "fa fa-arrow-down", "moveDown", "gridButtonReadOnly", "order",
                                        null);
                            ...
                            $scope.$bus.subscribe('buckslip-task-object-updated', function(objectInfo) {
                                ...
                                if (!Util.isEmpty(objectInfo.id)) {
                                    ComplaintFutureApprovalService.getBuckslipProcessesForChildren("COMPLAINT", objectInfo.id).then(
                                            function(response) {
                                    ...
                                    ComplaintFutureApprovalService.getBusinessProcessVariableForObject("COMPLAINT", objectInfo.id,
                                    ...
                                    ComplaintFutureApprovalService.getBuckslipProcessesForChildren(objectInfo.parentObjectType,
                            ...
                            $scope.initiateTask = function() {
                                if (!Util.isEmpty($scope.buckslipProcess)) {
                                    ...
                                    ComplaintFutureApprovalService
                                            .updateBuckslipProcess($scope.buckslipProcess)
                                            .then(
                                                    function(result) {
                                                        ComplaintFutureApprovalService
                                                                .initiateRoutingWorkflow($scope.buckslipProcess.businessProcessId)
                                                                .then(
                                                                        function(result) {
                                                                            cleanCachedCaseFile($stateParams.id);
                                                                            $timeout(
                                                                                    function() {
                                                                                        $scope.$emit('report-object-refreshed',
                                                                                                $stateParams.id);
                                                                                        MessageService
                                                                                                .info($translate
                                                                                                        .instant('complaints.comp.approvalRouting.processInitialize.successfull'));
                                                                        ...
                                                                        function(reason) {
                                                                            MessageService
                                                                                    .error($translate
                                                                                            .instant('complaints.comp.approvalRouting.processInitialize.fail'));
                                                                        });
                                                    },
                                                    function(reason) {
                                                        MessageService.error($translate
                                                                .instant('complaints.comp.approvalRouting.businessProcessUpdate.fail'));
                                                    });
                                }
                            };
                            ...
                            $scope.withdrawTask = function() {
                                if (!Util.isEmpty($scope.taskInfo.taskId)) {
                                    ...
                                    ComplaintFutureApprovalService.withdrawRoutingWorkflow($scope.taskInfo.taskId).then(
                                            function(result) {
                                                cleanCachedCaseFile($stateParams.id);
                                                $timeout(function() {
                                                    ...
                                                    MessageService.info($translate
                                                            .instant('complaints.comp.approvalRouting.processWithdraw.successfull'));
                                            ...
                                            function(reason) {
                                                MessageService.error($translate
                                                        .instant('complaints.comp.approvalRouting.processWithdraw.fail'));
                                            });
                                }
                            };
                            ...
                            $scope.saveTask = function() {
                                ComplaintFutureApprovalService.updateBuckslipProcess($scope.buckslipProcess).then(
                                        function(result) {
                                            $scope.oldData = angular.copy($scope.gridOptions.data);
                                            MessageService.info($translate
                                                    .instant('complaints.comp.approvalRouting.businessProcessUpdate.successfull'));
                                        },
                                        function(reason) {
                                            MessageService.error($translate
                                                    .instant('complaints.comp.approvalRouting.businessProcessUpdate.fail'));
                            ...
                            var configureCellTemplate = function(clickFn, icon, readOnlyFn, tooltip, name) {
                                var cellTemplate = "<a class='inline animated btn btn-default btn-xs'" + " ng-click='grid.appScope."
                                        + clickFn + "(row.entity)'";
