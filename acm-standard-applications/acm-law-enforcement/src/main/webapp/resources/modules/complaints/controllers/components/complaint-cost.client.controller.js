...
angular
        ...
        .controller(
                ...
                [
                        ...
                        function($scope, $stateParams, $translate, $state, Util, ObjectService, ConfigService, ObjectCostService,
                                ComplaintInfoService, HelperUiGridService, HelperObjectBrowserService) {
                            ...
                            var onObjectInfoRetrieved = function(objectInfo) {
                                if (Util.goodPositive(componentHelper.currentObjectId, false)) {
                                    ...
                                    ObjectCostService.queryCostsheets(ObjectService.ObjectTypes.COMPLAINT, componentHelper.currentObjectId)
                                            .then(
                                                    function(costsheets) {
                                                        componentHelper.promiseConfig.then(function(config) {
                                                            for (var i = 0; i < costsheets.length; i++) {
                                                                costsheets[i].acm$_formName = $translate
                                                                        .instant("complaints.comp.cost.formNamePrefix")
                                                                        + " " + Util.goodValue(costsheets[i].parentNumber);
                                                                costsheets[i].acm$_costs = _.reduce(Util.goodArray(costsheets[i].costs),
                                                                        function(total, n) {
