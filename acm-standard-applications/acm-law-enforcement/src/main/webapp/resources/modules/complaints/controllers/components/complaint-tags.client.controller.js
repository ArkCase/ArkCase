...
angular.module('complaints')
        .controller(
                ...
                [
                        ...
                        '$modal',
                        'MessageService',
                        function($scope, $q, $stateParams, $translate, Util, ObjectService, ConfigService, ObjectCostService,
                                ComplaintInfoService, HelperUiGridService, HelperObjectBrowserService, ObjectTagsService, $modal,
                                messageService) {
                                            ...
                                            if (tag.object_id_s) {
                                                ...
                                                if (tagsFound.length == 0) {
                                                    ObjectTagsService.associateTag(componentHelper.currentObjectId,
                                                            ObjectService.ObjectTypes.COMPLAINT, $scope.objectParentTitle, tag.object_id_s)
                                                    ...
                                                    messageService.info(tag.tags_s + " "
                                                            + $translate.instant('complaints.comp.tags.message.tagAssociated'));
                                                ...
                                                ObjectTagsService.associateTag(componentHelper.currentObjectId,
                                                        ObjectService.ObjectTypes.COMPLAINT, $scope.objectParentTitle, tag.id).then(
                                                        function() {
                                                            $scope.tags.push(tag);
                                                            $scope.gridOptions.data = $scope.tags;
                                                            $scope.gridOptions.totalItems = $scope.tags.length;
                            ...
                            $scope.deleteRow = function(rowEntity) {
                                ...
                                if (0 < id) { //do not need to call service when deleting a new row with id==0
                                    ObjectTagsService.removeAssociateTag(componentHelper.currentObjectId,
                                            ObjectService.ObjectTypes.COMPLAINT, rowEntity.id).then(function() {
